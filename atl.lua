-- BSS AI v15.7 — FINAL (Timers Update, FIFO CH, X10 Purples fix)
...
TKS[1629649299] = { n = "Focus", base = 4, p = 15 }
TKS[65867881] = { n = "Haste", base = 4, p = 15 }
TKS[1442863423] = { n = "Blue Boost", base = 4, p = 12 }
TKS[1442859163] = { n = "Red Boost", base = 4, p = 12 }
TKS[3877732821] = { n = "White Boost", base = 4, p = 12 }
TKS[1442764904] = { n = "Red Bomb+", base = 4, p = 12 }
TKS[1442700745] = { n = "Rage", base = 8, p = 10 }
TKS[253828517] = { n = "Melody", base = 8, p = 10 }
TKS[2499514197] = { n = "Honey Mark", base = 8, p = 9 }
TKS[2499540966] = { n = "Pollen Mark", base = 8, p = 9 }
...
TKS[1874564120] = { n = "Pulse", base = 12, p = 7 }
TKS[4528379338] = { n = "Mark Surge", base = 4, p = 7 }
TKS[3582501342] = { n = "Rain Call", base = 24, p = 6 }
TKS[3582519526] = { n = "Tornado", base = 24, p = 6 }
TKS[5877998606] = { n = "Mind Hack", base = 16, p = 6 }
TKS[8083943936] = { n = "Surprise Party", base = 24, p = 6 }
TKS[177997841] = { n = "Glob", base = 4, p = 6 }
TKS[1839454544] = { n = "Gummy Storm", base = 4, p = 6 }
TKS[1442725244] = { n = "Bomb", base = 4, p = 5 }
...
TKS[4519549299] = { n = "Inferno", base = 4, p = 5 }
TKS[4519523935] = { n = "Triangulate", base = 4, p = 5 }
TKS[4528414666] = { n = "Summon Frog", base = 8, p = 5 }
TKS[4528208186] = { n = "Flame Fuel", base = 8, p = 5 }
TKS[1671281844] = { n = "Beamstorm", base = 12, p = 4 }
TKS[8083436978] = { n = "Blue Balloon", base = 4, p = 4 }
TKS[1104415222] = { n = "BondToken", base = 4, p = 4 }
TKS[2319100769] = { n = "Fetch", base = 8, p = 4 }
TKS[4889322534] = { n = "Fuzz Bombs", base = 4, p = 4 }
TKS[2319083910] = { n = "Impale", base = 24, p = 4 }
TKS[3080529618] = { n = "Jelly Bean", base = 4, p = 4 }
TKS[4889470194] = { n = "Pollen Haze", base = 4, p = 4 }
TKS[107187190] = { n = "Honey Gift", base = 4, p = 2 }
TKS[183390139] = { n = "Cog", base = 4, p = 2 }
...
local function createTimer(part, id, totalLifetime, duped, def)
    ...
    label.TextColor3 = duped and (def.dupedColor or def.normalColor or Color3.new(1,1,1)) or (def.normalColor or Color3.new(1,1,1))
...
-- Изменено: теперь при X10 он полностью фильтрует серые метки, если мы запрашиваем только фиолетовые
...
local function rT(o)
    ...
    if dp or id == SMI or id == 1629547638 or id == 8173559749 then
...
-- Heartbeat loop for the visible 3D timers above tokens
...
local function gAWB()
    ...
    if p_ == "X10" then
        if isSuperOutside then
            ...
            -- X10: Только фиолетовые, сортировка по времени (FIFO)
    ...
    if p_ == "НАБОР" then
        ...
        -- НАБОР: Все кросхеиры строго по времени появления (FIFO), без принудительной сортировки фиолетовых вперед
...
local function uVC()
    ...
    if ENABLED and r then
        ...
        if not _G.patrolVis then
            pcall(function()
                local pv = Instance.new("Part")
                pv.Name = "PatrolVis"
                pv.Shape = Enum.PartType.Cylinder
                pv.Anchored = true
                pv.CanCollide = false
                pv.CanQuery = false
                pv.Transparency = 0.8
        ...
        if _G.patrolVis then
            local cc = Vector3.zero
            if curF and curF.part then cc = curF.part.Position end
            if aR and aR.Parent then cc = aR.Position end
            local pr = (aR and aR.Parent) and aRR or 15
            _G.patrolVis.Size = Vector3.new(pr * 2, 0.1, pr * 2)
            if cc ~= Vector3.zero then
                _G.patrolVis.CFrame = CFrame.new(cc.X, cc.Y + 0.05, cc.Z) * CFrame.Angles(0, 0, math.pi/2)
            else
                _G.patrolVis.CFrame = CFrame.new(r.Position.X, r.Position.Y - 2, r.Position.Z) * CFrame.Angles(0, 0, math.pi/2)
            end
        end
    else
        if syVis then pcall(function() syVis:Destroy() end); syVis = nil end
        if _G.patrolVis then pcall(function() _G.patrolVis:Destroy() end); _G.patrolVis = nil end
    end
end
        ...
        local function rR()
            if isSuperScorch then
                ...
                for fl, _ in pairs(scytheParts) do
                    if fl.Parent then
                        local isDark = (fl.Name:find("Dark") or fl.BrickColor.Name == "Really black")
                        local weight = isDark and darkWeight or 1
                        cx = cx + (fl.Position.X * weight)
                        cz = cz + (fl.Position.Z * weight)
                        count = count + weight
                    end
                end
                local center = Workspace.Flowers:FindFirstChild("FP18-10-13")
                if center then
                    local cw = 5
                    cx = cx + (center.Position.X * cw)
                    cz = cz + (center.Position.Z * cw)
                    count = count + cw
                end
                if count > 0 then
                    cx = cx / count
