--[[
  BSS AI v15.3-FIXED
  v14.7 working skeleton + CoreStats + ScorchRec + BUILD TP + AntiLag
  ALL 6 BUGS FIXED:
  1. Scorch detection: flatBuffs stores by Src string too
  2. CH looting: gCH pcall-safe, tryCollectNearbyCH no blocking
  3. TokenLink at XFlame 20+: skip if far from center
  4. XFlame 20+ patrol: stays near center (XCR*0.8)
  5. Scythe hit: scanScythes + tryHitScythe via ToolCollect/VIM
  6. go_crosshair: doesn't skip during REFRESH, xfE CH distance check
]]
...
local function logErr(m)
  ...
  if elbl then
    local L = {}
    for i = math.max(1, #elog - 12), #elog do
...
mkErrGui()
logOk("GUI ошибок запущен")

-- ============================================================
-- 3. CONFIG
-- ============================================================
local SB = { ["НАБОР"] = 70, ["X10"] = 90, ["REFRESH"] = 75 }
local SJ = 3
local AM = 1.2
local DGL = 22
local FM = 3
...
TKS[2499514197] = { n = "Honey Mark", b = 8, p = 7 }
TKS[2499540966] = { n = "Pollen Mark", b = 8, p = 7 }
...
TKS[8173559749] = { n = "Target Practice", b = 8, p = 3 }
...
aB.FC = { combo = 0, dur = 20, tL = 0 }
aB.RB = { combo = 0, dur = 15, tL = 0 }
...
local function getCtxKey()
  local fcS = "F0"
  if aB.FC.combo >= 10 then
    fcS = "F1"
  end
  local rbS = "R0"
  if aB.RB.combo >= 10 then
    rbS = "R1"
  end
...
local function fldHashFn()
  ...
  return string.format("%.0f_%.0f", c.X / 50, c.Z / 50)
end
  ...
  if z then
    ...
    for _, zn in ipairs(z:GetChildren()) do
      if zn:IsA("BasePart") then
        ...
        local s = zn.Size
        if math.abs(mp.X - zn.Position.X) <= s.X / 2 + 20 and math.abs(mp.Z - zn.Position.Z) <= s.Z / 2 + 20 then
          if d < bd then
            bd = d
            be = zn
          end
...
-- ============================================================
-- 8. CROSSHAIRS (FIXED: pcall-safe Parent check, inline cleanup)
-- ============================================================
...
local function iP(p)
  ...
  if ok and c and iCl(c, PURP) then
    return true
  end
...
local function clnCH()
  for i = #cQ, 1, -1 do
    if not cQ[i].part.Parent or cQ[i].col then
      table.remove(cQ, i)
    end
  end
end

-- FIXED gCH: pcall-safe Parent check + inline dead cleanup
...
local function gTPG_build()
  ...
  while i <= #all - 2 do
    ...
    local b = all[i + 1]
    local c = all[i + 2]
    if not a.isP and not b.isP and c.isP then
      if c.sT - a.sT <= 2 then
        table.insert(G, { r1 = a, r2 = b, pr = c })
        i = i + 3
      else
        i = i + 1
      end
...
local function gRCT(mP, dP)
  if focusRenew or isSuperScorch then
    return {}
  end
  if not prec.isX or prec.nR then
    return {}
...
local function cAT(mP, dP)
  ...
  local ap = t.pos + bp * CAS
  st.chA = st.chA + 1
  return cP(Vector3.new(ap.X, mP.Y, ap.Z))
end
...
local function gDTC()
  ...
  for p, t in pairs(aT) do
    if not t.col and t.dp and p.Parent then
      if d3(r.Position, p.Position) < 80 then
        c = c + 1
      end
    end
  end
...
-- ============================================================
-- 10. PATH CH COLLECTION (FIXED: no blocking while loop)
-- ============================================================
local function tryCollectNearbyCH(orig)
  local r = h()
  if not r then
    return
  end
  for _, ch in ipairs(cQ) do
    if not ch.col and ch.part.Parent and ch.part ~= orig then
...
-- ============================================================
-- 13. BUFFS (FIXED: flatBuffs now stores by Src string too)
-- ============================================================
local rps = nil
do
...
-- FIXED: also index by Src field for passives like Scorching Star Aura
...
local function sBf()
  ...
  -- FIXED: Scorching Star Aura found by Src string now (flatBuffs stores Src→table)
  ...
  -- FIXED: X-Flame Aura found by Src string now
  ...
  aB.PM.a = false
  if fd[2575093099] then
    aB.PM.a = rawget(fd[2575093099], "Removed") ~= true
  end

  local pm = fd[PMBI]
  if pm and rawget(pm, "Removed") ~= true then
    aB.PoM.a = true
    aB.PoM.m = tonumber(rawget(pm, "Value") or 0) or 1
    if aR then
...
local function sPr()
  ...
  if b and rawget(b, "Removed") ~= true then
    ...
    local bStart = tonumber(b.Start)
    if bStart and bStart ~= prec.sS then
      prec.sS = bStart
      prec.sD = tonumber(b.Dur) or 60
      prec.ls = os.clock()
    end
    prec.st = math.min(PMX, math.round(val / PPK))
    prec.val = val
...
-- ============================================================
-- 16. HISTORY + PATTERN MGMT + SCYTHE
-- ============================================================
local function uHB()
  local n = tick()
  ...
  local cut = n - 300
  while #aBf > 0 and aBf[1].time < cut do
    table.remove(aBf, 1)
  end
end
    ...
    for _, pa in ipairs(p.actions) do
      if pa.action == action then
        if d2Sq(pos, pa.pos) < 100 then
          local ms = 1
          if pa.phase == p_ then ms = ms * 2 end
          if pa.isSc == isSc then ms = ms * 2 end
...
local function recalcTop10()
  local now = tick()
  local recent = {}
  for _, s in ipairs(scorchSessions) do
    if now - s.time <= PAT_WINDOW then
      table.insert(recent, s)
    end
  end
  table.sort(recent, function(a, b) return a.honeyGained > b.honeyGained end)
  top10patterns = {}
  for i = 1, math.min(PAT_TOP, #recent) do
    top10patterns[i] = recent[i]
  end
end
...
local function getPatternBias(action)
  ...
  for _, tp in ipairs(top10patterns) do
    if tp.ctx == ctx then
      for _, ta in ipairs(tp.actions) do
        if ta.action == action then
          if SCORCH_BIAS > bias then
            bias = SCORCH_BIAS
          end
          break
        end
...
-- SCYTHE HIT (FIXED: scan + hit via ToolCollect/VIM)
...
local function tryHitScythe()
  ...
  for fl, data in pairs(scytheParts) do
    if not data.hit and fl.Parent then
      local dist = d3(r.Position, fl.Position)
      if dist <= SCYTHE_DIST and n - data.sT >= 0.3 then
        lastScytheHit = n
        ...
        local hit = false
        if tce then
          local ok = pcall(function() tce:FireServer() end)
          if ok then
            hit = true
          end
        end
        ...
        task.wait(0.1)
        local bg2 = r:FindFirstChild("AI_BG_Scythe")
        if bg2 then
          bg2:Destroy()
        end
        if hit then
...
local function gDTP()
  local n = tick()
  for p, t in pairs(aT) do
    if not t.col and p.Parent and t.id == TPI and t.dp then
      if t.l - (n - t.s) > 1 then
        return p, t
      end
    end
...
local function gSDA()
  ...
  for p, t in pairs(aT) do
    if not t.col and p.Parent then
      if t.id == SMI and d3(p.Position, rp) <= aRR * 1.5 then
        local d = d3(r.Position, p.Position)
        if d < bdSm then
          bdSm = d
          bpSm = p
      ...
      if t.id == TPI and t.dp and d3(p.Position, rp) <= aRR * 1.5 then
        local d = d3(r.Position, p.Position)
        if d < bdTp then
          bdTp = d
          bpTp = p
...
local function eS()
  ...
  local sU = "0"
  if smT ~= nil then sU = "1" end
  local hP = "0"
  if #fP > 0 then hP = "1" end
  local nT = false
  local n = tick()
  for p, t in pairs(aT) do
    if not t.col and p.Parent then
      if (t.l - (n - t.s)) > 1 and d3(r.Position, p.Position) < 30 then
        nT = true
        break
      end
    end
  ...
  if prec.isX and not prec.nR then
    ...
    for _, ch in ipairs(cQ) do
      if not ch.col and ch.part.Parent and not ch.isP then
        if d3(r.Position, ch.part.Position) < 20 then
          ct = ct + 1
        end
      end
    end
  ...
  local sc = "0"
  if aB.SS.st > 0 then sc = "1" end
  local xf = "0"
  if aB.XF.st >= 20 then xf = "1" end
  local fcS = "0"
  if aB.FC.combo >= 10 then fcS = "1" end
  local rbS = "0"
  if aB.RB.combo >= 10 then rbS = "1" end
...
local function gAWB()
  ...
  if isSuperScorch then
    ...
    if rbScorch then
      local pp = gPCH()
      if #pp > 0 then
        return { "go_purple_rb_renew" }
      end
      local all = gCH(false, false)
      if #all > 0 then
        return { "go_skip_one_ch" }
      end
    end
    local pp = gPCH()
    if #pp >= 2 then
      return { "go_multi_purple" }
    end
    local all = gCH(false, false)
    if #all > 0 then
      table.insert(ba, "go_crosshair_all")
    end
    if #ba > 0 then
      return ba
    end
  ...
  if p_ == "REFRESH" then
    local all = gCH(false, false)
    if #all > 0 then
      table.insert(ba, "go_crosshair_refresh_all")
    else
  ...
  if p_ == "X10" then
    local tp = gTPG()
    if tp then
      table.insert(ba, "go_target_practice_purple")
    else
      local pp = gCH(true, false)
      if #pp > 0 then
        table.insert(ba, "go_purple")
      end
    end
  end
...
local function uXC()
  if xfE then
    ...
    if not xfC then
      xfC = Instance.new("Part")
      xfC.Name = "XFlameCircle"
      xfC.Shape = Enum.PartType.Cylinder
      xfC.Anchored = true
      xfC.CanCollide = false
      xfC.Transparency = 0.6
      pcall(function() xfC.BrickColor = BrickColor.new("Really red") end)
      xfC.Size = Vector3.new(XCR * 2, 0.2, XCR * 2)
      xfC.Parent = Workspace
    end
    xfC.Position = Vector3.new(cc.X, cc.Y + 0.2, cc.Z)
  else
    if xfC then
      xfC:Destroy()
      xfC = nil
...
local function iCH(p)
  if not p or p.Name ~= "Crosshair" then
    return false
  end
  local ok, c = pcall(function() return p.Color end)
  if not ok then
  ...
  return math.abs(c.R - CHC.R) < 0.05
     and math.abs(c.G - CHC.G) < 0.05
     and math.abs(c.B - CHC.B) < 0.05
end

-- ============================================================
-- 20. EXECUTE (ALL FIXES APPLIED)
-- ============================================================
local function eA(action)
  ...
  if action == "go_build_precision" then
    ...
    if g.r1.part.Parent and not g.r1.col then
      ...
      if ok and g.r1.part.Parent then
        g.r1.col = true
        st.ch = st.ch + 1
        rw = rw + 10
        task.wait(0.05)
    ...
    if g.r2.part.Parent and not g.r2.col then
      ...
      if ok and g.r2.part.Parent then
        g.r2.col = true
        st.ch = st.ch + 1
        rw = rw + 10
      end
    ...
    if rw > 0 then return rw else return -2 end

  elseif action == "go_focus_renew" then
    local all = gCH(false, false)
  ...
  elseif action == "go_purple_rb_renew" then
    local pp = gPCH()
  ...
  elseif action == "go_skip_one_ch" then
    local all = gCH(false, false)
  ...
  elseif action == "go_multi_purple" then
    local pp = gPCH()
  ...
  elseif action == "go_crosshair_refresh_all" then
    ...
    if col > 0 then return col * 12 else return -2 end
  ...
  elseif action == "go_smile_area" then
    ...
    if ok and t.Parent then
      ...
      while tick() - st_ < TSD do
        task.wait(0.1)
        if not t.Parent or INT then break end
        local hp = h()
        if hp then
          hp.CFrame = CFrame.new(t.Position.X, hp.Position.Y, t.Position.Z)
        end
      end
      td.col = true
      st.sm = st.sm + 1
  ...
  elseif action == "go_smile" then
    ...
    tL = "😊 Smile"
    INT = false
    local ok = goTo(t.Position, 4, math.min(2.5, smTR - 0.2))
    if ok and t.Parent then
      local st_ = tick()
      while tick() - st_ < TSD do
        task.wait(0.1)
  ...
  elseif action == "go_purple" then
    ...
    for _, ch in ipairs(pp) do
      if ch.part.Parent and not ch.col and not smT and not prec.nR then
  ...
  elseif action == "go_tokenlink" then
    -- FIXED: skip TokenLink far from center during xfE
    ...
    for p, t in pairs(aT) do
      if not t.col and p.Parent and t.p >= 90 then
        if xfE and cc ~= Vector3.zero then
          local distToCenter = d3(p.Position, cc)
          if distToCenter > XCR * 2 then
            -- too far from XFlame zone, skip
          else
            tL = "💎Link"
            INT = false
  ...
  elseif action == "go_crosshair" then
    -- FIXED: don't skip during REFRESH, xfE CH distance check
    local all = gCH(false, false, ph() == "НАБОР")
    if #all == 0 then return -1 end
    local t = all[1]
    local rw = 0
    INT = false
    if t.part.Parent and not t.col and not smT then
      ...
      if r2 then
        for p2, t2 in pairs(aT) do
          if not t2.col and p2.Parent and t2.p >= 90 then
            if d3(r2.Position, p2.Position) < TLID and d3(r2.Position, t.part.Position) > 30 then
              sk = true
              break
            end
          end
      ...
      -- FIXED: xfE — skip CH too far from center
      if xfE and not sk then
        local cc = gFC()
        if cc ~= Vector3.zero then
          local chDist = d3(t.part.Position, cc)
          if chDist > XCR * 1.5 then
            sk = true
          end
        end
      end
  ...
  elseif action == "go_dup_tp" then
    ...
    if ok and p.Parent then
      ...
      while tick() - st_ < TSD do
        task.wait(0.1)
        if not p.Parent or INT then break end
        local hp = h()
        if hp then
          hp.CFrame = CFrame.new(p.Position.X, hp.Position.Y, p.Position.Z)
        end
      end
      t.col = true
      return 15
  ...
  elseif action == "go_petal" then
    ...
    if ca then return tr else return -1 end
  ...
  elseif action == "go_token_best" then
    ...
    for p, t in pairs(aT) do
      if not t.col and p.Parent then
        if (t.l - (n - t.s)) > 0.5 and t.p > bP then
          be = p
          bP = t.p
        end
      end
  ...
  elseif action == "patrol_ring" then
    -- FIXED: xfE — stay near center (XCR*0.8 radius)
    local function rR()
      if xfE then
        local cc = gFC()
        if cc ~= Vector3.zero then
          local a_ = math.random() * 2 * math.pi
          local rr_ = math.random() * XCR * 0.8
      ...
      elseif curF then
        ...
        local rp = h()
        if rp then
          return cP(rp.Position + Vector3.new((math.random() * 2 - 1) * 30, 0, (math.random() * 2 - 1) * 30))
        end
        return cP(Vector3.zero)
      end
    end
    tL = "🚶 Ring"
    if xfE then tL = "🔥 XF Ring" end
    INT = false
    local t = rR()
    if t == Vector3.zero or (not xfE and not iF(t)) then
      t = gFC()
    end
    if t == Vector3.zero then return 0 end
    goTo(t, xfE and 3 or 6, PT)
  ...
  elseif action == "patrol_random" then
    ...
    INT = false
    local t = rF()
    if t == Vector3.zero or not iF(t) then
      t = gFC()
    end
    if t == Vector3.zero then return 0 end
    goTo(t, 4, PT)
...
-- ============================================================
-- 21. HEARTBEAT (FIXED: scythe scan + hit added)
-- ============================================================
local mLS = false
...
local function sML()
  ...
  fAR()
  fF()
  logOk("BSS AI v15.3-fixed ready! " .. fmtHoney(scriptStartHoney))
  print("✅ v15.3-fixed")
  tL = "иниц"
  lMT = tick()
end
...
if _G._BSSAI_HB then
  pcall(function() _G._BSSAI_HB:Disconnect() end)
end
...
_G._BSSAI_HB = RunService.Heartbeat:Connect(function()
  ...
  -- FIXED: Scythe hit (before isA check, high priority)
  pcall(tryHitScythe)
  ...
  if prec.isX and not prec.nR and r then
    for _, ch in ipairs(cQ) do
      if ch.part and ch.part.Parent and not ch.col and not ch.isP then
        if iCH(ch.part) and n - lPT > 1.5 then
          lPT = n
          local s_ = eS()
...
hl.Position = UDim2.new(0, 0, 0, 38)
...
sl.Position = UDim2.new(0, 0, 0, 54)
...
local pl = Instance.new("TextLabel", fr)
pl.Size = UDim2.new(1, 0, 0, 16)
pl.Position = UDim2.new(0, 0, 0, 70)
pl.BackgroundTransparency = 1
pl.Text = "Φ: --"
pl.TextColor3 = Color3.fromRGB(180, 180, 255)
pl.Font = Enum.Font.Gotham
pl.TextSize = 11
pl.TextXAlignment = Enum.TextXAlignment.Center

local bf = Instance.new("TextLabel", fr)
bf.Size = UDim2.new(1, 0, 0, 16)
bf.Position = UDim2.new(0, 0, 0, 86)
...
sh.Position = UDim2.new(0, 0, 0, 102)
...
task.spawn(function()
  while true do
    ...
    sl.Text = "⚡ " .. string.format("%.0f", cS)
    local superTag = ""
    if isSuperScorch then superTag = "🔥 " end
    pl.Text = superTag .. ph() .. " CH:" .. #cQ .. " Tk:" .. st.tk .. " Pr:" .. st.pr
    ...
    bf.Text = "FC:" .. fcLabel .. " " .. string.format("%.0f", aB.FC.tL) .. "с RB:" .. rbLabel .. " " .. string.format("%.0f", aB.RB.tL) .. "с PM:" .. aB.PoM.m
...
UIS.InputBegan:Connect(function(i, gp)
  ...
  elseif i.KeyCode == Enum.KeyCode.P then
    ...
    print("v15.3 Φ:" .. ph() .. " ε:" .. string.format("%.3f", EP) .. " R:" .. st.tR .. " S:" .. c .. " FC:" .. aB.FC.combo .. " RB:" .. aB.RB.combo .. " PM:" .. aB.PoM.m .. " Honey:" .. fmtHoney(getCoreHoney()) .. " Sc:" .. #scorchSessions)
  end
end)
