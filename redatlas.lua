-- BSS AI v15.1 — SCORCH PATTERN RECORDING + Q-BIAS (Lua 5.1 tested)
...
local Q_VERSION,ENABLED="15.1",true
...
t.Text="⚠ BSS AI v15.1 — Error Log";t.TextColor3=Color3.fromRGB(255,180,60);t.Font=Enum.Font.GothamBold;t.TextSize=14;t.TextXAlignment=Enum.TextXAlignment.Left
Instance.new("Frame",bg).Size=UDim2.new(1,-16,0,1);Instance.new("Frame",bg).Position=UDim2.new(0,8,0,36);Instance.new("Frame",bg).BackgroundColor3=Color3.fromRGB(80,80,100);Instance.new("Frame",bg).BorderSizePixel=0
...
local PRELOAD_BIAS=1.3
local PAT_WINDOW=2400 -- 40 минут в секундах
local PAT_SAVE=120 -- каждые 2 минуты
local PAT_TOP=10
...
local SCO_TIPS={
  ...
  "📝 Не забывай ставить новые Precision Marks",
  "📝 Если Digital копирует морф — дотяни до Scorch для х2 морфа",
  "📝 Focus x10 + Red Boost x10 = база. Бот следит автоматически"
}
local scoIdx=0
...
-- v15.1: HONEY через CoreStats
local function getCoreHoney()
  local cs=LP:FindFirstChild("CoreStats")
  if cs then local hv=cs:FindFirstChild("Honey")
...
-- v15.1: получить контекст-ключ для паттернов
...
-- === BUFFS (v15.1: scorch recording logic) ===
...
local function sBf() if not rps then return end;local ok,res=pcall(rps.InvokeServer,rps);if not ok or type(res)~="table"then return end
  ...
  -- v15.1: SCORCH RECORDING LOGIC
  local curHoney=getCoreHoney()
  if aB.SS.st>0 and prevSS==0 then
    -- SCORCH STARTED
    scorchStartHoney=curHoney
    scorchStartTime=tick()
    scorchActive=true
    scorchRecording=true
    scorchActions={}
  elseif aB.SS.st==0 and prevSS>0 then
    -- SCORCH ENDED — финализируем сессию
    scorchActive=false
    if scorchRecording and scorchStartTime>0 then
      local gained=curHoney-scorchStartHoney
      local dur=(tick()-scorchStartTime)/60
      if gained>0 then
        local ctx=getCtxKey()
        local session={honeyGained=gained,durationMin=math.floor(dur*10)/10,time=tick(),ssCombo=prevSS,ctx=ctx,actions=scorchActions}
        table.insert(scorchSessions,session)
        if gained>bestScorchHoney then bestScorchHoney=gained end
        -- сохраняем в pH для совместимости с gPB
        table.insert(pH,{hps=gained,scorchHoney=gained,scorchDur=dur,actions=scorchActions,timestamp=tick(),isScorch=true,ctx={phase=ph(),scorch=1}})
        table.sort(pH,function(a,b)return(a.hps or a.scorchHoney or 0)>(b.hps or b.scorchHoney or 0)end)
...
local function uHB()
  ...
  local cut=n-300;while#aBf>0 and aBf[1].time<cut do table.remove(aBf,1)end
end

-- v15.1: пересчёт топ-10 паттернов (40-мин окно)
local function recalcTop10()
  ...
  top10patterns={}
  for i=1,math.min(PAT_TOP,#recent)do top10patterns[i]=recent[i]end
end

-- v15.1: сохранение скорч-сессий в файл (каждые 2 мин)
local function saveScorchSessions()
  if tick()-lastPatternSave<PAT_SAVE then return end
  lastPatternSave=tick()
  recalcTop10()
  pcall(function()if writefile then writefile("bss_ai_scorch_v15.json",Http:JSONEncode({version=Q_VERSION,scorchSessions=scorchSessions,bestScorch=bestScorchHoney,top10=top10patterns,time=tick()}))end end)
end

-- v15.1: Pattern Bias — проверить есть ли действие в топ-паттернах с похожим контекстом
local function getPatternBias(action)
  local ctx=getCtxKey()
  local bias=1.0
  -- проверяем preloaded (хардкод) паттерны
  for _,pp in ipairs(preloadPatterns)do
    if pp.ctx==ctx then for _,pa in ipairs(pp.actions)do if pa.action==action then bias=math.max(bias,PRELOAD_BIAS);break end end end
  end
  -- проверяем топ-10
  for _,tp in ipairs(top10patterns)do
    if tp.ctx==ctx then for _,ta in ipairs(tp.actions)do if ta.action==action then bias=math.max(bias,SCORCH_BIAS);break end end end
  end
  return bias
end

-- v15.1: запись действия во время скорча
local function recordScorchAction(action)
  if not scorchRecording then return end
  local r=h()
  table.insert(scorchActions,{action=action,pos=r and r.Position or Vector3.zero,phase=ph(),fc=(aB.FC.combo>=10 and 1 or 0),rb=(aB.RB.combo>=10 and 1 or 0),pm=math.min(3,aB.PoM.m),ssCombo=aB.SS.st})
end
...
-- v15.1: cAB с Pattern Bias
local function cAB(s)
  local v=gAWB();if#v==0 then return"patrol_ring"end
...
-- === ИСПОЛНЕНИЕ (v15.1: recordScorchAction на каждом eA) ===
local function eA(action)
  ...
  if action=="go_build_precision"then local tp=gTPG_build();if not tp then return-1 end;local g=tp[1];local rw=0;INT=false
    if g.pr.part.Parent and not g.pr.col then tL="🟣 BUILD TP (purple)";local ok=goTo(g.pr.part.Position,4,5);if ok and g.pr.part.Parent then g.pr.col=true;lP=g.pr.part;st.pr=st.pr+1;rw=rw+30;task.wait(0.1)end end
    if g.r1.part.Parent and not g.r1.col then tL="🎯 BUILD TP (r1)";local ok=goTo(g.r1.part.Position,4,4);if ok and g.r1.part.Parent then g.r1.col=true;st.ch=st.ch+1;rw=rw+10;task.wait(0.05)end end
    if g.r2.part.Parent and not g.r2.col then tL="🎯 BUILD TP (r2)";local ok=goTo(g.r2.part.Position,4,4);if ok and g.r2.part.Parent then g.r2.col=true;st.ch=st.ch+1;rw=rw+10 end end
  ...
  if action=="go_purple_rb_renew"then local pp=gPCH();if#pp==0 then return-1 end;local t=pp[1];tL="🟣 RB renew (purple)";INT=false;local ok=goTo(t.part.Position,4,4);if ok and t.part.Parent then t.col=true;st.pr=st.pr+1;return 20 end;return-2 end
  ...
  if action=="go_multi_purple"then local pp=gPCH();if#pp<2 then return-1 end;local rw=0;INT=false
    ...
    local p2=pp[2];if p2.part.Parent and not p2.col then tL="🟣×2 turbo #2"
      ...
      else local ok=goTo(p2.part.Position,4,3);if ok and p2.part.Parent then p2.col=true;st.pr=st.pr+1;rw=rw+20 end
      end
  ...
  if action=="go_crosshair_all"then local all=gCH(false,false);if#all==0 then return-1 end;table.sort(all,function(a,b)return a.sT<b.sT end);local rw=0;INT=false
    for _,ch in ipairs(all)do if ch.part.Parent and not ch.col then if ch.isP then tL="🟣 Scorch CH";local ok=goTo(ch.part.Position,4,3);if ok and ch.part.Parent then ch.col=true;st.pr=st.pr+1;rw=rw+20 end
    else tL="🎯 Scorch CH";local ok=goTo(ch.part.Position,4,3);if ok and ch.part.Parent then ch.col=true;st.ch=st.ch+1;rw=rw+10 end end end
    return rw>0 and rw or-2 end
...
local function sML() if mLS then return end;mLS=true;task.wait(2)
  ...
  lQ();fAR();fF();logOk("BSS AI v15.1 готов! Honey: "..fmtHoney(scriptStartHoney));print("✅ BSS AI v15.1 ready.");tL="иниц";lMT=tick()end
task.spawn(function()pcall(function()local ok,raw=pcall(readfile,pF);if ok and raw then local ok2,data=pcall(Http.JSONDecode,Http,raw);if ok2 and type(data)=="table"then if data.scorchSessions then scorchSessions=data.scorchSessions else pH=data end;if data.bestScorch then bestScorchHoney=data.bestScorch end;for _,p in ipairs(data.actions or data)do if(p.hps or p.scorchHoney or 0)>bAH then bAH=(p.hps or p.scorchHoney or 0)end end end end end);logOk("Скорч-сессии загр: "..#scorchSessions)end)
...
_G._BSSAI_HB=RunService.Heartbeat:Connect(function()
  ...
  if hbF%18000==0 then task.spawn(function()local qc=0;for _ in pairs(QT)do qc=qc+1 end;pcall(writefile,"bss_ai_q_v15.json",Http:JSONEncode({version=Q_VERSION,qtable=QT,scorchSessions=scorchSessions,bestScorch=bestScorchHoney,top10=top10patterns,meta={sc=qc,sa=os.time(),tR=math.floor(st.tR),dc=st.dc}}))end)end
...
local function rQ() QT={};EP=0.1;st.tR=0;st.dc=0;scorchSessions={};bestScorchHoney=0;top10patterns={};pcall(writefile,"bss_ai_q_v15.json",Http:JSONEncode({version=Q_VERSION,qtable={},scorchSessions={},bestScorch=0,top10={},meta={ra=os.time()}}))end

-- === GUI (v15.1: scorch stats + pattern count) ===
...
local ti_=Instance.new("TextLabel",fr)ti_.Size=UDim2.new(1,0,0,20)ti_.Position=UDim2.new(0,0,0,2)ti_.BackgroundTransparency=1 ti_.Text="🧠 BSS AI v15.1"ti_.TextColor3=Color3.fromRGB(100,200,255)ti_.Font=Enum.Font.GothamBold ti_.TextSize=12 ti_.TextXAlignment=Enum.TextXAlignment.Center
...
task.spawn(function()while true do task.wait(0.3)
  ...
  hl.Text="HPH: "..fmtHoney(hph).."/час | Honey: "..fmtHoney(curH)
  sl_.Text="⚡ спид: "..string.format("%.0f",cS)
  pl_.Text=(isSuperScorch and"🔥 SUPER "or"")..ph().." | CH:"..#cQ.." | Тк:"..st.tk.." | Pr:"..st.pr
  bf_.Text="FC:"..(aB.FC.combo>=10 and"x10 "or"x"..aB.FC.combo)..string.format("%.0f",aB.FC.tL).."с RB:"..(aB.RB.combo>=10 and"x10 "or"x"..aB.RB.combo)..string.format("%.0f",aB.RB.tL).."с PM:"..aB.PoM.m.."/3"
  if scorchActive and scorchStartTime>0 then
    local scorchElapsed=(tick()-scorchStartTime)/60
    local scorchGained=curH-scorchStartHoney
    sh_.Text="🔥 Скорч: "..fmtHoney(scorchGained).." за "..string.format("%.1f",scorchElapsed).."мин | Лучший: "..fmtHoney(bestScorchHoney)
  else
    sh_.Text="🔥 Скорчей: "..#scorchSessions.." | Топ10: "..#top10patterns.." | Лучший: "..fmtHoney(bestScorchHoney)
  end
  if hbF%180==0 then scoIdx=(scoIdx%#SCO_TIPS)+1;tip_.Text=SCO_TIPS[scoIdx]end
end end)
UIS.InputBegan:Connect(function(i,gp)if gp then return end;if i.KeyCode==Enum.KeyCode.T then ENABLED=not ENABLED;sg.Enabled=ENABLED elseif i.KeyCode==Enum.KeyCode.G then rQ()elseif i.KeyCode==Enum.KeyCode.P then local c=0;for _ in pairs(QT)do c=c+1 end;local curH=getCoreHoney();print("Φ:"..ph().." ε:"..string.format("%.3f",EP).." R:"..st.tR.." S:"..c.." P:"..#pH.." FC:"..aB.FC.combo.." RB:"..aB.RB.combo.." PM:"..aB.PoM.m.." Honey:"..fmtHoney(curH).." ScorchSessions:"..#scorchSessions.." Top10:"..#top10patterns)end end)
LP.CharacterAdded:Connect(function()task.wait(2)aT={};cQ={};lP=nil;curF=nil;tL="старт";smT=nil;isCS=false;INT=false;cyc={chC=0};fP={};igT=0;rCC=0;dupCnt=0;focusRenew=false;rbSkip=false;isSuperScorch=false;scorchActive=false;scorchRecording=false;scorchActions={};scorchStartHoney=0;scorchStartTime=0;if xfC then xfC:Destroy();xfC=nil end;task.spawn(function()local qc=0;for _ in pairs(QT)do qc=qc+1 end;pcall(writefile,"bss_ai_q_v15.json",Http:JSONEncode({version=Q_VERSION,qtable=QT,scorchSessions=scorchSessions,bestScorch=bestScorchHoney,top10=top10patterns,meta={sc=qc,sa=os.time()}}))end)end)
print("✅ v15.1 готов.")
