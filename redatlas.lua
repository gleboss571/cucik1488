-- BSS AI v12.9: CH превыше всего, зелёные CH детект, X-Flame фикс, смайл фикс
...
local b=Instance.new("Frame",eg);b.Size=UDim2.new(0,380,0,220);b.Position=UDim2.new(.5,-190,.5,-110)
...
t.Text="⚠ BSS AI v12.9 — Статус";t.TextColor3=Color3.fromRGB(255,180,60);t.Font=Enum.Font.GothamBold;t.TextSize=14;t.TextXAlignment=Enum.TextXAlignment.Left
local l=Instance.new("Frame",b);l.Size=UDim2.new(1,-16,0,1);l.Position=UDim2.new(0,8,0,36);l.BackgroundColor3=Color3.fromRGB(80,80,100);l.BorderSizePixel=0
el=Instance.new("TextLabel",b);el.Size=UDim2.new(1,-16,0,115);el.Position=UDim2.new(0,8,0,42);el.BackgroundTransparency=1
el.Text="⏳ Иниц...";el.TextColor3=Color3.fromRGB(200,200,200);el.Font=Enum.Font.Code;el.TextSize=11
el.TextXAlignment=Enum.TextXAlignment.Left;el.TextYAlignment=Enum.TextYAlignment.Top;el.TextWrapped=true;el.RichText=true
eb=Instance.new("TextButton",b);eb.Size=UDim2.new(0,100,0,28);eb.Position=UDim2.new(0,8,0,168)
eb.BackgroundColor3=Color3.fromRGB(60,60,80);eb.BorderSizePixel=0;eb.Text="📋 Копировать";eb.TextColor3=Color3.fromRGB(220,220,220);eb.Font=Enum.Font.Gotham;eb.TextSize=11
Instance.new("UICorner",eb).CornerRadius=UDim.new(0,4)
local cb=Instance.new("TextButton",b);cb.Size=UDim2.new(0,80,0,28);cb.Position=UDim2.new(0,116,0,168)
...
eb.MouseButton1Click:Connect(function()local tx=table.concat(eL,"\n");if#tx==0 then tx="Нет ошибок"end;pcall(setclipboard,tx);eb.Text="✅ Скоп.";task.wait(1.5);eb.Text="📋 Копировать"end)
task.spawn(function()task.wait(4);if eg and ec==0 then eg.Enabled=false;eg:Destroy();eg=nil end end)end)end
...
local TPI,TLC,PT,XCR,SCRD,PCHR=8173559749,2,8,12,4,10
...
local cyc={chC=0};local st={tk=0,ch=0,pr=0,x10=0,rf=0,tR=0,dc=0,sm=0,chA=0,pt=0,fl=0,chP=0,chG=0}
...
-- ДЕТЕКТ ЗЕЛЁНЫХ КРОСХЕИРОВ (на которые уже наступили)
local function isGreen(part)
    ...
    return math.abs(c.R-CHIT_COLOR.R)<0.08 and math.abs(c.G-CHIT_COLOR.G)<0.08 and math.abs(c.B-CHIT_COLOR.B)<0.08
end

local function updateGreenStatus()
    for _,ch in ipairs(cQ)do
        if not ch.col and ch.part.Parent then
            if isGreen(ch.part)and not ch.wasGreen then
                ch.wasGreen=true;ch.greenTime=tick()
                if not ch.isP and prec.isX and not prec.nR then st.chG=st.chG+1 end
            end
        end
    end
end
...
local function gCH(op,oR,incGreen)
    local L_={}
    for _,ch in ipairs(cQ)do
        if not ch.col and ch.part.Parent then
            if not incGreen and ch.wasGreen then continue end
            if(op and ch.isP)or(oR and not ch.isP)or(not op and not oR)then table.insert(L_,ch)end
        end
    end
    table.sort(L_,function(a,b)return a.sT<b.sT end)
    return L_
end

local function gCHall(op,oR)
    local L_={}
    for _,ch in ipairs(cQ)do if not ch.col and ch.part.Parent then if(op and ch.isP)or(oR and not ch.isP)or(not op and not oR)then table.insert(L_,ch)end end end
    table.sort(L_,function(a,b)return a.sT<b.sT end)
    return L_
end

local function gCHuntouched(op,oR)return gCH(op,oR,false)end
...
-- === СБОР CH ПО ПУТИ ===
local function tCN(pos)local r=hr();if not r then return end;local rp=r.Position
    for _,ch in ipairs(cQ)do if not ch.col and ch.part.Parent and not ch.wasGreen then if d3(rp,ch.part.Position)<=PCHR then
        local h_=hm();if h_ then h_:MoveTo(ch.part.Position)end;local t0=tick()
        while tick()-t0<1.2 do task.wait(0.04);local r2=hr();if not r2 or not ch.part.Parent then break end
            if d3(r2.Position,ch.part.Position)<=4 then ch.col=true;st.chP=st.chP+1;if ch.isP then st.pr=st.pr+1;lP=ch.part else st.ch=st.ch+1 end;break
            end
        end;break
    end end end
end
...
local function goTo(tP,rad,to,sk)
    ...
    h_:MoveTo(cM);local t0=tick();local lM=tick();local lA=tick();local lPC=tick()
    while tick()-t0<to do task.wait(0.04)
        ...
        if tick()-lPC>=0.25 then lPC=tick();tCN(nil)end
...
-- === СПИРАЛЬ ФЛЕЙМОВ ===
local SPR_STEP=2.2
local function gST()local r=hr();if not r then return nil end
    if not sprC or tick()-lSM>2 then lSM=tick();sprC=gFCC();if sprC then sprA=0;sprR=1 end end
    if not sprC then return nil end
    local step=SPR_STEP/math.max(sprR,1);sprA=sprA+step;sprR=1+sprA*SPR_STEP/(2*math.pi)
    if sprR>SCRD*3 then sprA=0;sprR=1 end
...
W.DescendantAdded:Connect(function(o)task.wait(0.05)pcall(rT,o)end)
for _,o in ipairs(W:GetDescendants())do pcall(rT,o)end
...
local function sPt()fP={};if not EN or not curF then return end;local pt=W:FindFirstChild("Particles");if not pt then return end;local r=hr();if not r then return end
...
-- === SMILE (всегда бежать когда под AreaRing и 6+ duped) ===
local function sSm()local n=tick();smT=nil;smTR=0;local bp,bd,br=nil,math.huge,0;local r=hr();if not r then return end;local rp=aR and aR.Position
    for p,t in pairs(aT)do if not t.col and p.Parent and t.id==SMI then local rem=t.l-(n-t.s);if rem<=SMRT and rem>0 then
        local tk=false;if rp then if d3(p.Position,rp)<=aRR*1.5 then tk=true end else tk=true end
        if gDTC()>=6 and tk then local d=d3(r.Position,p.Position);if d<bd then bp=p;bd=d;br=rem end end
    end end end
...
local function uHB()local n=tick();local hps=gHP();if hps>0 then table.insert(hB,{time=n,hps=hps})end;local cut=n-120;while#hB>0 and hB[1].time<cut do table.remove(hB,1)end
    if tL and tL~="старт"then local r=hr();table.insert(aBf,{time=n,action=tL,pos=r and r.Position or Vector3.zero,phase=ph(),isSc=aB.SS.st>0})end
    ...
    if n-laT>=10 then laT=n;local sum,cn=0,0;for _,e in ipairs(hB)do sum=sum+e.hps;cn=cn+1 end;local av=cn>0 and(sum/cn)or 0
        if av>0 then local ac={};for _,e in ipairs(aBf)do table.insert(ac,{action=e.action,pos=e.pos,to=e.time-(n-120),phase=e.phase,isSc=e.isSc})end;table.insert(pH,{hps=av,actions=ac,timestamp=n});table.sort(pH,function(a,b)return a.hps>b.hps end);while#pH>30 do table.remove(pH)end;if av>bAH then bAH=av end;pcall(function()writefile(pF,H:JSONEncode(pH))end)end
    end
end
...
local function eS()local r=hr();if not r then return"dead"end;local p_=ph();local tlD="none"
    ...
    local prN=math.min(3,#gCH(true,false));local rN=math.min(3,#gCH(false,true));local sU=(smT~=nil);local hP_=(#fP>0);local nT=false;local n=tick()
    ...
    return string.format("PH:%s|TL:%s|CH:%d|PR:%d|SM:%s|NT:%s|Z:%s|CT:%s|PT:%s|SC:%s|XF:%s",p_,tlD,rN,prN,tostring(sU),tostring(nT),zn,chT,tostring(hP_),sc,xf)
end

-- === ДЕЙСТВИЯ (CH ПРЕВЫШЕ ВСЕГО) ===
local function gAWB()
    local ba={};local p_=ph();local n=tick()
    local untCH=gCHuntouched(false,false)
    if#untCH>0 then
        if p_=="X10"then local tp=gTPG();if tp then return{"go_target_practice_purple"}end;local pp=gCHuntouched(true,false);if#pp>0 then return{"go_purple"}end end
        if p_=="REFRESH"then local all=gCHall(false,false);if#all>0 then table.insert(ba,"go_crosshair_refresh_all")else table.insert(ba,"patrol_ring")end;return ba end
    ...
    if xfE then local cc=gFC();if cc~=Vector3.zero then local bC,bD=nil,XCR+1
        for _,ch in ipairs(cQ)do if not ch.col and ch.part.Parent and not ch.wasGreen then local d=d3(ch.part.Position,cc);if d<=XCR and d<bD then bD=d;bC=ch end end end
        if bC then return{"go_xflame_ch"}else return{"go_xflame_center"}end
    end end
    if#fP>0 then table.insert(ba,"go_petal")end
    local all=gCH(false,false);if#all>0 then table.insert(ba,"go_crosshair")end
    local ha=false;for _ in pairs(aT)do ha=true;break end
    if ha then table.insert(ba,"go_token_near");table.insert(ba,"go_token_best")end
    local dt=gDTP();if dt then table.insert(ba,"go_dup_tp")end
    table.insert(ba,"patrol_ring")
    if p_=="НАБОР"then table.insert(ba,"patrol_random")end
    return ba
end
...
local function dUQ(s,a,rw,ns)local rr=hr();local tR=rw+gPB(a,rr and rr.Position or Vector3.zero);local v=gAWB();local mN=0;for _,a_ in ipairs(v)do local q=gQ(ns,a_);if q>mN then mN=q end end;local nw=gQ(s,a)+AL*(tR+GA*mN-gQ(s,a));sQ(s,a,nw);st.tR=st.tR+tR;st.dc=st.dc+1;EP=math.max(0.02,EP*ED)end
...
local function eA(action)
    ...
    if action=="go_scorching_spiral"then local t=gST();if not t then return-1 end;tL="🔥 Спираль";INT=false;goTo(t,2,2);task.wait(0.12);return 2
    elseif action=="go_crosshair_refresh_all"then local all=gCHall(false,false);if#all==0 then return-1 end;local col=0;INT=false
        for _,ch in ipairs(all)do if ch.part.Parent and not ch.col then if col>=3 then break end;if ch.wasGreen then continue end;tL="🔄 REFRESH ("..(col+1).."/3)";local ok=goTo(ch.part.Position,4,4,true);if ok and ch.part.Parent then ch.col=true;rCC=rCC+1;if ch.isP then st.pr=st.pr+1;lP=ch.part else st.ch=st.ch+1 end;col=col+1;task.wait(0.1)end end end
        if aR and aR.Parent then tL="🏠 возвр в кольцо";goTo(aR.Position,6,4)else goTo(cP(r.Position),5,2)end
    ...
    elseif action=="go_target_practice_purple"then local tp=gTPG();if not tp then return-1 end;local rw=0;INT=false
        for _,g in ipairs(tp)do if g.pr.part.Parent and not g.pr.col and not g.pr.wasGreen then tL="🎯 TP фиол";local ok=goTo(g.pr.part.Position,4,5);if ok and g.pr.part.Parent then g.pr.col=true;g.r1.col=true;g.r2.col=true;lP=g.pr.part;st.pr=st.pr+1;tL="🟣 TP Precise Mark";local t0=tick();while tick()-t0<PST do task.wait(0.05);if smT or prec.nR or not EN then break end end;rw=rw+40 end end end;return rw>0 and rw or-2
    ...
    elseif action=="go_purple"then local pp=gCHuntouched(true,false);if#pp==0 then return-1 end;local rw=0;INT=false;for _,ch in ipairs(pp)do if ch.part.Parent and not ch.col and not smT and not prec.nR and not ch.wasGreen then local ok=goTo(ch.part.Position,4,5);if ok and ch.part.Parent then ch.col=true;lP=ch.part;st.pr=st.pr+1;tL="🟣 1с";local t0=tick();while tick()-t0<PST do task.wait(0.05);if smT or prec.nR or not EN then break end end;rw=rw+30 end end end;return rw>0 and rw or-2
    ...
    elseif action=="go_crosshair"then local all=gCHuntouched(false,false);if#all==0 then all=gCH(false,false,true);if#all==0 then return-1 end end;local t=all[1];local rw=0;INT=false
        if t.part.Parent and not t.col and not smT and not prec.nR then local sk=false;local r2=hr()
            ...
            if not sk then local ok=goTo(t.part.Position,4,5);if ok and t.part.Parent then t.col=true;if t.isP then st.pr=st.pr+1;lP=t.part;rw=rw+10 else st.ch=st.ch+1;cyc.chC=cyc.chC+1;if cyc.chC>=3 then cyc.chC=0 end;rw=rw+8 end end end
        end
    ...
    elseif action=="go_xflame_ch"then local c=gFC();if c==Vector3.zero then return-1 end;local bC,bD=nil,XCR+1;for _,ch in ipairs(cQ)do if not ch.col and ch.part.Parent and not ch.wasGreen then local d=d3(ch.part.Position,c);if d<=XCR and d<bD then bD=d;bC=ch end end end;if bC then tL="🔥 XFlame CH";INT=false;goTo(bC.part.Position,2,3);task.wait(1);return 5 end;return-1
    end
    return 0
end
...
-- === ГЛАВНЫЙ HEARTBEAT ===
local mLS=false
local function sML()if mLS then return end;mLS=true;task.wait(2);lQ();fAR();fF();lI("BSS AI v12.9 готов!");print("✅ BSS AI v12.9 safe+ готов.");tL="иниц";lMT=tick()end
task.spawn(function()pcall(function()local ok,raw=pcall(readfile,pF);if ok and raw then local ok2,data=pcall(H.JSONDecode,H,raw);if ok2 and type(data)=="table"then pH=data;for _,p in ipairs(pH)do if p.hps and p.hps>bAH then bAH=p.hps end end end end end);lI("Паттерны загр.")end)

R.Heartbeat:Connect(function()
    hbF=hbF+1;if not EN then return end
    if not mLS then sML();return end
    local n=tick()
    fAR();updateGreenStatus()
    ...
    if hbF%18000==0 then pcall(function()local qc=0;for _ in pairs(QT)do qc=qc+1 end;pcall(writefile,"bss_ai_q_v12.json",H:JSONEncode({version=VQ,qtable=QT,meta={sc=qc,ep=math.floor(EP*1000)/1000,uid=tostring(L.UserId),sa=os.time(),tR=math.floor(st.tR),dc=st.dc}}))end)end
...
function lQ()local ok,raw=pcall(readfile,"bss_ai_q_v12.json");if ok and raw then local ok2,d=pcall(H.JSONDecode,H,raw);if ok2 and type(d)=="table"and d.version==VQ and type(d.qtable)=="table"then QT=d.qtable end end end
local function rQ()QT={};EP=0.1;st.tR=0;st.dc=0;pcall(writefile,"bss_ai_q_v12.json",H:JSONEncode({version=VQ,qtable={},meta={ra=os.time()}}))end

-- === GUI ===
local sg=Instance.new("ScreenGui",G)sg.Name="BSSAI_GUI"
local fr=Instance.new("Frame",sg)fr.Size=UDim2.new(0,255,0,105)fr.Position=UDim2.new(0,10,0,10)
...
task.spawn(function()while true do task.wait(0.3)lb.Text="🎯 "..tL;local cur=gHP();local hs=cur>0 and string.format("%.0f",cur)or"--";local bs=bAH>0 and string.format("%.0f",bAH)or"--";hl.Text="HPS: "..hs.." | Рек: "..bs;sl_.Text="⚡ "..string.format("%.0f",cS);pl_.Text="Φ:"..ph().." CH:"..#cQ.." Тк:"..st.tk.." Зл:"..st.chG.." ПЧ:"..st.chP end end)
U.InputBegan:Connect(function(i,gp)if gp then return end;if i.KeyCode==Enum.KeyCode.T then EN=not EN;sg.Enabled=EN elseif i.KeyCode==Enum.KeyCode.G then rQ()elseif i.KeyCode==Enum.KeyCode.P then local c=0;for _ in pairs(QT)do c=c+1 end;print("Φ:"..ph().." ε:"..string.format("%.3f",EP).." R:"..st.tR.." S:"..c.." P:"..#pH.." Зел:"..st.chG)end end)
L.CharacterAdded:Connect(function()task.wait(2)aT={};cQ={};lP=nil;curF=nil;tL="старт";smT=nil;isCS=false;INT=false;cyc={chC=0};tF={};fP={};igT=0;rCC=0;sprC=nil;sprA=0;sprR=0;if xfC then xfC:Destroy();xfC=nil end;if scC then scC:Destroy();scC=nil end;pcall(function()local qc=0;for _ in pairs(QT)do qc=qc+1 end;pcall(writefile,"bss_ai_q_v12.json",H:JSONEncode({version=VQ,qtable=QT,meta={sc=qc,sa=os.time()}}))end)end)
print("✅ Готово.")
