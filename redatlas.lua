-- BSS AI v12.7 safe + Red Bomb Plus + REFRESH fix + фиол. стойка только при X10
...
local PETAL_COLORS = {
    ["Red"] = Color3.fromRGB(249, 34, 34),
    ["Pink"] = Color3.fromRGB(255, 130, 201),
    ["Merigold"] = Color3.fromRGB(218, 168, 28),
    ["Periwinkle"] = Color3.fromRGB(150, 156, 236),
    ["Violet"] = Color3.fromRGB(94, 38, 177),
    ["Scarlet"] = Color3.fromRGB(171, 19, 19),
    ["Green"] = Color3.fromRGB(35, 232, 5),
    ["Yellow"] = Color3.fromRGB(238, 204, 79),
    ["Black"] = Color3.fromRGB(11, 11, 11),
    ["Grey"] = Color3.fromRGB(127, 127, 127),
    ["Blue"] = Color3.fromRGB(33, 66, 249),
    ["Cyan"] = Color3.fromRGB(29, 196, 222),
...
local activeBuffs = {
    ScorchingStar = {stacks = 0},
    XFlame = {stacks = 0},
    PreciseMark = {active = false},
    PollenMark = {active = false, pos = nil}
}
local stuckPetals = {}
...
local function getPhase()
    if not prec.isX10 then
        return "НАБОР"
    elseif prec.needRefresh then
        return "REFRESH"
    else
        return "X10"
    end
...
local function getFieldCenter()
    if curField and curField.part then
        return curField.part.Position
    end
    local r = getHRP()
...
local function isPurple(part)
    ...
    if ok and c and isClose(c, PURPLE) then
        return true
    end
    local ok2, bc = pcall(function()
        return part.BrickColor.Color
    end)
    return ok2 and bc and isClose(bc, PURPLE)
end

local function addCH(obj)
    if obj.Name ~= "Crosshair" or not obj:IsA("BasePart") then
        return
    end
    ...
    table.insert(chQueue, {
        part = obj,
        spawnTime = tick(),
        collected = false,
        isPurple = isPurple(obj)
    })
end
...
if Parts then
    ...
    Parts.DescendantRemoving:Connect(function(obj)
        for i = #chQueue, 1, -1 do
            if chQueue[i].part == obj then
                table.remove(chQueue, i)
                break
            end
        end
        if lastPurple == obj then
            lastPurple = nil
        end
    end)
    for _, o in ipairs(Parts:GetDescendants()) do
        addCH(o)
    end
end
...
local function getRegCHThreats(myPos, destPos)
    if not prec.isX10 or prec.needRefresh then
        return {}
    end
...
local function getDupedTokenCount()
    ...
    for part, t in pairs(activeTokens) do
        if not t.collected and t.duped and part.Parent then
            if dist3(r.Position, part.Position) < 80 then
                c = c + 1
            end
        end
    end
...
local function goTo(targetPos, radius, timeout, skipClamp)
    ...
    if avoid then
        curMove = Vector3.new(avoid.X, r.Position.Y, avoid.Z)
    end
    h:MoveTo(curMove)
    local t0 = tick()
    local lastMove = tick()
    ...
    while tick() - t0 < timeout do
        ...
        if dist3(r.Position, origTarget) <= radius then
            return true
        end
        if tick() - lastAvoid >= stepCooldown then
            ...
            if newAvoid then
                curMove = Vector3.new(newAvoid.X, r.Position.Y, newAvoid.Z)
            else
                curMove = origTarget
            end
        end
...
do
    ...
    if e then
        toolCollectEvent = e:FindFirstChild("ToolCollect")
    end
end
    ...
    if pf then
        ...
        if pf:IsA("ColorSequenceValue") then
            ...
            if seq and seq.Keypoints and #seq.Keypoints > 0 then
                ...
                if key and type(key) == "table" and key.Value then
                    color = key.Value
                elseif key and type(key) == "userdata" and key.Value then
                    color = key.Value
                end
            end
        ...
        if color then
            return color.G < 0.3 and color.B > 0.5
        end
    end
    local ok, c = pcall(function()
        return flm.Color
    ...
    while true do
        ...
        if flames then
            ...
            for flm in pairs(trackedFlames) do
                if not seen[flm] then
                    trackedFlames[flm] = nil
                end
            end
        end
    end
...
local function regToken(obj)
    if obj.Name ~= "C" or not obj:IsA("BasePart") or activeTokens[obj] or tick() < ignoreNewTokensUntil then
        return
    end
    ...
    if duped then
        life = life * (2 + 0.05 * (DIG_BEE_LVL - 1))
    end
    activeTokens[obj] = {
        id = id,
        name = def.name,
...
game.DescendantRemoving:Connect(function(obj)
    if activeTokens[obj] then
        if activeTokens[obj].collected then
    ...
    while true do
        ...
        if ok and type(result) == "table" then
            ...
            local function findPreciseMark(tbl)
                ...
                if rawget(tbl, "BuffID") == 2575093099 and rawget(tbl, "Removed") ~= true then
                    return true
                end
                for _, v in pairs(tbl) do
                    if findPreciseMark(v) then
                        return true
                    end
                end
                return false
            ...
            if pm and pm > 0 then
                ...
                if areaRing then
                    activeBuffs.PollenMark.pos = areaRing.Position
                end
            else
...
local function getFireClusterCenter()
    ...
    for flm in pairs(trackedFlames) do
        if flm.Parent then
            table.insert(flames, flm.Position)
        end
    end
    if #flames == 0 then
        return nil
...
local function getPetalColor(part)
    for name, col in pairs(PETAL_COLORS) do
        if (col.R - part.Color.R) ^ 2 + (col.G - part.Color.G) ^ 2 + (col.B - part.Color.B) ^ 2 < 0.002 then
            return name
        end
    end
    return nil
...
task.spawn(function()
    while true do
        task.wait(0.15)
        fieldPetals = {}
        if not ENABLED or not curField then
            return
        end
        local particles = Workspace:FindFirstChild("Particles")
        ...
        for _, obj in ipairs(particles:GetChildren()) do
            if obj.Name == "PetalPart" and obj:IsA("BasePart") and isInField(obj.Position) then
                ...
                if cName and PETAL_PRIORITY[cName] then
                    ...
                    table.insert(fieldPetals, {
                        part = obj,
                        colorName = cName,
                        priority = PETAL_PRIORITY[cName],
                        dist = d
                    })
                end
            end
...
local function getHoneyPerSecond()
    if not hpsMeterLabel or not hpsMeterLabel.Parent then
        hpsMeterLabel = findHPSLabel()
    end
    if hpsMeterLabel and hpsMeterLabel:IsA("TextLabel") then
        return parseHPS(hpsMeterLabel.Text)
    end
    return 0
end
    ...
    if ok and raw then
        ...
        if ok2 and type(data) == "table" then
            ...
            for _, p in ipairs(patternsHistory) do
                if p.hps and p.hps > maxHPS then
                    maxHPS = p.hps
                end
            end
            bestAvgHPS = maxHPS
...
local function updateHPSBuffers()
    ...
    if hps > 0 then
        table.insert(hpsBuffer, {time = now, hps = hps})
    end
    local cutoff = now - HPS_WINDOW
    while #hpsBuffer > 0 and hpsBuffer[1].time < cutoff do
        table.remove(hpsBuffer, 1)
    end
    if taskLabel and taskLabel ~= "старт" then
        ...
        table.insert(actionBuffer, {
            ...
            pos = pos,
            phase = getPhase(),
            isScorch = activeBuffs.ScorchingStar.stacks > 0
        })
    end
    while #actionBuffer > 0 and actionBuffer[1].time < cutoff do
    ...
    if now - lastAnalyzeTime >= HPS_ANALYZE_INTERVAL then
        ...
        local sum = 0
        local count = 0
        for _, e in ipairs(hpsBuffer) do
            sum = sum + e.hps
            count = count + 1
        end
        ...
        if avg > 0 then
            ...
            for _, e in ipairs(actionBuffer) do
                table.insert(acts, {
                    action = e.action,
                    pos = e.pos,
                    timeOffset = e.time - (now - HPS_WINDOW),
                    phase = e.phase,
                    isScorch = e.isScorch
                })
            end
            table.insert(patternsHistory, {hps = avg, actions = acts, timestamp = now})
            table.sort(patternsHistory, function(a, b)
                return a.hps > b.hps
            end)
            while #patternsHistory > MAX_PATTERNS do
                table.remove(patternsHistory)
            end
            if avg > bestAvgHPS then
                bestAvgHPS = avg
            end
            savePatterns()
        end
...
task.spawn(function()
    while true do
        ...
        for _, t in pairs(activeTokens) do
            if not t.collected and t.prio >= 90 then
                hasTL = true
                break
            end
        end
...
local function encodeState()
    ...
    for part, t in pairs(activeTokens) do
        if not t.collected and part.Parent then
            if (t.life - (n - t.spawn)) > 1 and dist3(r.Position, part.Position) < 30 then
                nearTok = true
                break
            end
        end
    ...
    if prec.isX10 and not prec.needRefresh then
        ...
        for _, ch in ipairs(chQueue) do
            if not ch.collected and ch.part.Parent and not ch.isPurple then
                if dist3(r.Position, ch.part.Position) < 20 then
                    ct = ct + 1
                end
            end
        end
    ...
    return string.format("PH:%s|TL:%s|CH:%d|PR:%d|SM:%s|NT:%s|Z:%s|CT:%s|PT:%s",
        ph, tlDist, regN, purN, tostring(smUrgent), tostring(nearTok), zone, chT, tostring(hasPetal))
end
...
local function getActionsWithBuffs()
    ...
    if smileTarget and getDupedTokenCount() > 6 then
        return {"go_smile"}
    end
    ...
    if activeBuffs.PollenMark.active and areaRing then
        ...
        if target then
            ...
            if t.id == SMILE_TOKEN_ID then
                table.insert(baseActions, 1, "go_smile_area")
            elseif t.id == TARGET_PRACTICE_ID and t.duped then
                table.insert(baseActions, 1, "go_dup_area")
            end
        end
    end
        ...
        if center ~= Vector3.zero then
            for _, ch in ipairs(chQueue) do
                if not ch.collected and ch.part.Parent then
                    ...
                    if d <= XFLAME_CENTER_RADIUS and d < closestDist then
                        closestDist = d
                        closestCH = ch
                    end
                end
    ...
    if phase == "REFRESH" then
        ...
        if #all > 0 then
            table.insert(baseActions, "go_crosshair_refresh")
        else
    ...
    if phase == "X10" then
        ...
        if #purps > 0 then
            table.insert(baseActions, "go_purple")
        end
    end

    if phase == "X10" or phase == "НАБОР" then
        if #fieldPetals > 0 then
            table.insert(baseActions, "go_petal")
        end
        local all = getCH(false, false)
        if #all > 0 then
            table.insert(baseActions, "go_crosshair")
        end
        local hasAny = false
        for _ in pairs(activeTokens) do
            hasAny = true
            break
        end
        if hasAny then
        ...
        if dtp then
            table.insert(baseActions, "go_dup_tp")
        end
        if activeBuffs.ScorchingStar.stacks > 0 and next(trackedFlames) then
            table.insert(baseActions, "go_scorching_center")
        end
        table.insert(baseActions, "patrol_ring")
        if phase == "НАБОР" then
            table.insert(baseActions, "patrol_random")
        end
        return baseActions
    end
...
task.spawn(function()
    while true do
        ...
        local function fp(t)
            ...
            if rawget(t, "BuffID") == PREC_BUFF_ID and rawget(t, "Removed") ~= true then
                return t
            end
            if rawget(t, "Src") == "Precision" then
                return t
            end
...
RunService.Heartbeat:Connect(function()
    if prec.localStart > 0 then
        ...
        if prec.needRefresh and refreshCHCounter == 0 then
            refreshStartTime = tick()
            refreshCHCounter = 0
        end
    end
end)
...
local function updateXFlameCircle()
    if xflameEmergency then
        ...
        if xflameCircle then
            xflameCircle:Destroy()
            xflameCircle = nil
        end
    end
end
...
local function updateScytheCircle()
    if ENABLED then
        ...
        if scytheCircle then
            scytheCircle:Destroy()
            scytheCircle = nil
        end
    end
end
...
local function getEdgePoint(targetPos)
    if not areaRing or not areaRing.Parent then
        return targetPos
    end
...
local function isCrosshairHit(part)
    if not part or part.Name ~= "Crosshair" then
        return false
    end
...
local function executeAction(action)
    ...
    if action == "go_smile_area" then
        ...
        local t = activeTokens[target]
        if not t or t.collected or t.id ~= SMILE_TOKEN_ID then
            return -1
        end
        ...
        if ok and target.Parent then
            ...
            while tick() - start < TOKEN_STAND_DUR do
                task.wait(0.1)
                if not target.Parent or INTERRUPT then
                    break
                end
                local hrp = getHRP()
                if hrp then
                    hrp.CFrame = CFrame.new(target.Position.X, hrp.Position.Y, target.Position.Z)
                end
            end
            t.collected = true
    ...
    elseif action == "go_dup_area" then
        ...
        local t = activeTokens[target]
        if not t or t.collected or t.id ~= TARGET_PRACTICE_ID or not t.duped then
            return -1
        end
    ...
    elseif action == "go_urgent_token" then
        ...
        for part, t in pairs(activeTokens) do
            if not t.collected and part.Parent then
                local rem = t.life - (now - t.spawn)
                if rem < bestLife and rem > 0 then
                    bestLife = rem
                    best = part
                end
            end
        ...
        if best then
            ...
            if ok and best.Parent then
                activeTokens[best].collected = true
                stats.tok = stats.tok + 1
                return 25
            end
            return -2
    ...
    elseif action == "go_crosshair_refresh" then
        ...
        for _, ch in ipairs(all) do
            if dist3(r.Position, ch.part.Position) < dist3(r.Position, target.part.Position) then
                target = ch
            end
        end
        taskLabel = "🔄 REFRESH"
        ...
        if ok and target.part.Parent then
            ...
            if target.isPurple then
                stats.purple = stats.purple + 1
                lastPurple = target.part
            else
                stats.ch = stats.ch + 1
            end
            ...
            if ch.part.Parent and not ch.collected and not smileTarget and not prec.needRefresh then
                ...
                if ok and ch.part.Parent then
                    ...
                    while tick() - t0 < PURPLE_STAND do
                        task.wait(0.05)
                        if smileTarget or prec.needRefresh or not ENABLED then
                            break
                        end
                    end
                    reward = reward + 30
    ...
    elseif action == "stay_purple" then
        if lastPurple and lastPurple.Parent and prec.isX10 and lastPurple:IsDescendantOf(workspace) then
            taskLabel = "🟣 стою"
            INTERRUPT = false
            goTo(lastPurple.Position, 3, 2)
            return 5
        else
            lastPurple = nil
            return -1
    ...
    elseif action == "go_crosshair" then
                ...
                if r2 then
                    for part2, t2 in pairs(activeTokens) do
                        if not t2.collected and part2.Parent and t2.prio >= 90 then
                            if dist3(r2.Position, part2.Position) < TL_INTERRUPT_DIST and dist3(r2.Position, ch.part.Position) > 30 then
                                shouldSkip = true
                                break
                            end
                        end
                ...
                if not shouldSkip then
                    ...
                    if ok and ch.part.Parent then
                        ...
                        if ch.isPurple then
                            ...
                            stats.ch = stats.ch + 1
                            cycle.chCollectedCount = cycle.chCollectedCount + 1
                            if cycle.chCollectedCount >= 3 then
                                cycle.chCollectedCount = 0
                            end
                            reward = reward + 8
                        end
        ...
        if reward > 0 and activeBuffs.PollenMark.active and activeBuffs.PollenMark.pos then
            taskLabel = "🏠 возврат в AreaRing"
            goTo(activeBuffs.PollenMark.pos, 6, 4)
        end
        return reward > 0 and reward or -2
    ...
    elseif action == "go_dup_tp" then
        ...
        if ok and part.Parent then
            ...
            while tick() - start < TOKEN_STAND_DUR do
                task.wait(0.1)
                if not part.Parent or INTERRUPT then
                    break
                end
                local hrp = getHRP()
                if hrp then
                    hrp.CFrame = CFrame.new(part.Position.X, hrp.Position.Y, part.Position.Z)
                end
            end
            t.collected = true
            return 15
    ...
    elseif action == "go_petal" then
        ...
        while i <= #fieldPetals do
            ...
            if not petal.part.Parent then
                table.remove(fieldPetals, i)
            elseif stuckPetals[petal.part] and tick() < stuckPetals[petal.part] then
                table.remove(fieldPetals, i)
            else
                taskLabel = "🌸 " .. petal.colorName
                INTERRUPT = false
    ...
    elseif action == "go_token_best" then
        ...
        for part, t in pairs(activeTokens) do
            if not t.collected and part.Parent then
                local rem = t.life - (now - t.spawn)
                if rem > 0.5 and t.prio > bestP then
                    best = part
                    bestP = t.prio
                end
            end
    ...
    elseif action == "patrol_ring" then
        local function rndRing()
            ...
            elseif curField then
                ...
                return Vector3.new(c.X + (math.random() * 2 - 1) * mx, 0, c.Z + (math.random() * 2 - 1) * mz)
            else
                local rp = getHRP()
                if rp then
                    return rp.Position + Vector3.new((math.random() * 2 - 1) * 30, 0, (math.random() * 2 - 1) * 30)
                else
                    return Vector3.zero
                end
            end
    ...
    elseif action == "go_scorching_center" then
        ...
        if dist3(r.Position, center) <= 5 then
            taskLabel = "🔥 Scorching (центр)"
            INTERRUPT = false
            task.wait(0.2)
            return 2
        else
    ...
    elseif action == "go_field_center" then
        local center = getFieldCenter()
        if center and center ~= Vector3.zero then
            taskLabel = "🎯 центр поля"
            goTo(center, 10, 3)
            return 0
        end
        return -1
    ...
    elseif action == "go_nearest_center_ch" then
        ...
        if bestCH and bestDist <= 10 then
            taskLabel = "🎯 CH центр"
            goTo(bestCH.part.Position, 4, 5)
            return 5
        end
        return -1
    ...
    elseif action == "go_xflame_ch" then
        ...
        for _, ch in ipairs(chQueue) do
            if not ch.collected and ch.part.Parent then
                local d = dist3(ch.part.Position, center)
                if d <= XFLAME_CENTER_RADIUS and d < bestDist then
                    bestDist = d
                    bestCH = ch
                end
            end
...
-- ===================== ГЛАВНЫЙ ЦИКЛ =====================
RunService.Heartbeat:Connect(function()
    if not ENABLED or not prec.isX10 or prec.needRefresh then
        return
    end
    local r = getHRP()
    if not r then
        ...
        if ch.part and ch.part.Parent and not ch.collected and not ch.isPurple then
            if isCrosshairHit(ch.part) and now - lastPenaltyTime > 1.5 then
                ...
                if state and state ~= "dead" then
                    pcall(doUpdateQ, state, "patrol_ring", -20, state)
                end
                stats.chAvoided = stats.chAvoided + 1
                break
            end
...
local function resetQ()
    ...
    stats.totalReward = 0
    stats.decisions = 0
    local ok, j = pcall(HttpService.JSONEncode, HttpService, {
        version = Q_VERSION,
        qtable = {},
        meta = {resetAt = os.time()}
    })
    if ok then
        pcall(writefile, Q_FILE, j)
    end
end
...
local function saveQ()
    ...
    local d = {
        ...
        meta = {
            stateCount = qc,
            epsilon = math.floor(EPSILON * 1000) / 1000,
            userId = tostring(LP.UserId),
            savedAt = os.time(),
            totalReward = math.floor(stats.totalReward),
...
task.spawn(function()
    ...
    loadQ()
    findAreaRing()
    print("✅ BSS AI v12.7 safe+ фиол.стойка только при X10. T/G/P")
    taskLabel = "инициализация"
    lastMoveTime = tick()
    while true do
        task.wait(0.03)
        if not ENABLED then
            ...
            if r then
                ...
                elseif tick() - lastMoveTime > 5 and not stuckWarning then
                    ...
                    INTERRUPT = true
                    taskLabel = "⏳ сброс"
                    if lastPurple and not lastPurple.Parent then
                        lastPurple = nil
                    end
                    if smileTarget and not smileTarget.Parent then
                        smileTarget = nil
                        isCollectingSmile = false
                    end
                    INTERRUPT = false
...
label.Text = "Действие: старт"
label.TextColor3 = Color3.fromRGB(255, 255, 255)
label.Font = Enum.Font.Gotham
label.TextSize = 13
label.TextXAlignment = Enum.TextXAlignment.Center
local hpsLabel = Instance.new("TextLabel", frame)
hpsLabel.Size = UDim2.new(1, 0, 0, 18)
hpsLabel.Position = UDim2.new(0, 0, 0, 44)
hpsLabel.BackgroundTransparency = 1
hpsLabel.Text = "HPS: -- | Рекорд: --"
hpsLabel.TextColor3 = Color3.fromRGB(150, 255, 150)
hpsLabel.Font = Enum.Font.Gotham
hpsLabel.TextSize = 12
hpsLabel.TextXAlignment = Enum.TextXAlignment.Center
task.spawn(function()
    while true do
        task.wait(0.3)
...
UserInputService.InputBegan:Connect(function(input, gp)
    ...
    elseif input.KeyCode == Enum.KeyCode.P then
        print("Phase: " .. getPhase() .. " ε:" .. string.format("%.3f", EPSILON) .. " R:" .. stats.totalReward)
        local c = 0
        for _ in pairs(QTable) do
            c = c + 1
        end
        print("States: " .. c)
        print("Patterns: " .. #patternsHistory)
    end
end)
