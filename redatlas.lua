...
-- === ДЕТЕКТ ДИАГОНАЛЕЙ (предикт линий CH) ===
-- Одна пчела спавнит 3 CH которые лежат на одной прямой (диагональ).
-- Когда пчела поднимается, линия искривляется — но spawnTime у всех 3 почти одинаковый.
-- Ищем группы из 3 CH: близкие по времени (≤0.8 сек) + примерно на одной линии.
local function getDiagonalGroups()
    local all=gCHall(false,false);if#all<3 then return nil end
    local groups={};local used={}
    for i=1,#all-2 do if not used[i]then
        for j=i+1,#all-1 do if not used[j]then
            if all[j].sT-all[i].sT<=DIAG_MAX_TIME then
                for k=j+1,#all do if not used[k]then
                    if all[k].sT-all[i].sT<=DIAG_MAX_TIME then
                        -- Проверка на коллинеарность: все 3 точки лежат на прямой?
                        local p1=all[i].part.Position;local p2=all[j].part.Position;local p3=all[k].part.Position
                        local v12=(p2-p1).Unit;local v13=(p3-p1).Unit
                        local dot=v12:Dot(v13)
                        -- Если dot близок к 1 или -1 — точки на одной прямой
                        if math.abs(math.abs(dot)-1)<DIAG_MAX_ANGLE then
                            used[i]=true;used[j]=true;used[k]=true
                            -- Определяем порядок: тот что дальше всех от центра — фиолетовый
                            local center=(p1+p2+p3)/3
                            local d1=(p1-center).Magnitude;local d2=(p2-center).Magnitude;local d3=(p3-center).Magnitude
                            local a,b,c=all[i],all[j],all[k]
                            if d1>=d2 and d1>=d3 then c,a,b=all[i],all[j],all[k]
                            elseif d2>=d1 and d2>=d3 then a,c,b=all[i],all[j],all[k]end
                            table.insert(groups,{r1=a,r2=b,pr=c,center=center})
                        end
                    end
                end end
            end
...
-- Возвращает ВСЕ диагонали (включая уже учтённые) — для REFRESH
local function getDiagonalForRefresh()
    local all=gCHall(false,false);if#all<3 then return nil end
    local groups={};local used={}
    for i=1,#all-2 do if not used[i]then
        for j=i+1,#all-1 do if not used[j]then
            if all[j].sT-all[i].sT<=1.5 then
                for k=j+1,#all do if not used[k]then
                    if all[k].sT-all[i].sT<=1.5 then
                        local p1,p2,p3=all[i].part.Position,all[j].part.Position,all[k].part.Position
                        local dot=(p2-p1).Unit:Dot((p3-p1).Unit)
                        if math.abs(math.abs(dot)-1)<0.5 then
                            used[i]=true;used[j]=true;used[k]=true
                            local c=(p1+p2+p3)/3
                            local d1,d2,d3=(p1-c).Magnitude,(p2-c).Magnitude,(p3-c).Magnitude
                            local a,b,cc=all[i],all[j],all[k]
                            if d1>=d2 and d1>=d3 then cc,a,b=all[i],all[j],all[k]
                            elseif d2>=d1 and d2>=d3 then a,cc,b=all[i],all[j],all[k]end
...
local function uHB()local n=tick();local hps=gHP();if hps>0 then table.insert(hB,{time=n,hps=hps})end;local cut=n-120;while#hB>0 and hB[1].time<cut do table.remove(hB,1)end;if tL and tL~="старт"then local r=hr();table.insert(aBf,{time=n,action=tL,pos=r and r.Position or Vector3.zero,phase=ph(),isSc=aB.SS.st>0})end;while#aBf>0 and aBf[1].time<cut do table.remove(aBf,1)end
...
local function gAWB()local ba={};local p_=ph();local n=tick()
    ...
    if#untCH>0 then if p_=="X10"then local tp=gTPG();if tp then return{"go_target_practice_purple"}end;local pp=gUNT(true,false);if#pp>0 then return{"go_purple"}end end
        if p_=="REFRESH"then
            -- REFRESH: ищем диагональ (3 CH одной пчелы)
            local di=getDiagonalForRefresh()
            if di then table.insert(ba,"go_crosshair_refresh_diagonal")else
                local all=gCHall(false,false);if#all>0 then table.insert(ba,"go_crosshair_refresh_all")else table.insert(ba,"patrol_ring")end
            end;return ba
...
local function eA(action)
    ...
    if action=="go_scorching_spiral"then local t=gST();if not t then return-1 end;tL="🔥 Спираль";INT=false;goTo(t,2,2);return 2
    elseif action=="go_crosshair_refresh_diagonal"then
        -- REFRESH по диагонали: собираем 3 CH ОДНОЙ пчелы
        local di=getDiagonalForRefresh();if not di then return-1 end
        local g=di[1];local col=0;INT=false
        -- Порядок: сначала два обычных, потом фиолетовый
        for _,ch in ipairs({g.r1,g.r2,g.pr})do if ch.part.Parent and not ch.col then
            if ch.wasGreen then continue end
            tL="🔄 REFRESH диаг ("..(col+1).."/3)"
            local ok=goTo(ch.part.Position,4,4,true)
            if ok==true and ch.part.Parent then ch.col=true;rCC=rCC+1
        ...
        if rCC>=3 then prec.nR=false;cyc.chC=0;st.rf=st.rf+1;rCC=0;tL="✅ REFRESH диаг OK";return 45 end
        return col>0 and(col*15)or-2
    elseif action=="go_crosshair_refresh_all"then local all=gCHall(false,false);if#all==0 then return-1 end;local col=0;INT=false
        for _,ch in ipairs(all)do if ch.part.Parent and not ch.col then if col>=3 then break end;if ch.wasGreen then continue end;tL="🔄 REFRESH ("..(col+1).."/3)";local ok=goTo(ch.part.Position,4,4,true);if ok==true and ch.part.Parent then ch.col=true;rCC=rCC+1;if ch.isP then st.pr=st.pr+1;lP=ch.part else st.ch=st.ch+1 end;col=col+1;task.wait(0.1)end end end
    ...
    elseif action=="go_smile"then local t=smT;if not t or not t.Parent then smT=nil;return-1 end;local td=aT[t];if not td or td.col then smT=nil;return-1 end;isCS=true;tL="😊 Smile";INT=false;local ok=goTo(t.Position,4,math.min(5,smTR-0.5));if ok==true and t.Parent then local st_=tick();while tick()-st_<TSD do task.wait(0.1);if not t.Parent or INT then break end;local hp=hr();if hp then hp.CFrame=CFrame.new(t.Position.X,hp.Position.Y,t.Position.Z)end end;td.col=true;smT=nil;st.sm=st.sm+1;isCS=false;return 45 end;isCS=false;smT=nil;return-10
    elseif action=="go_urgent_token"then local n=tick();local be,bl=nil,0.3;for p,t in pairs(aT)do if not t.col and p.Parent then local rem=t.l-(n-t.s);if rem<bl and rem>0 then bl=rem;be=p end end end;if be then tL="⚡Срочный";INT=false;local ok=goTo(be.Position,4,2);if ok then aT[be].col=true;st.tk=st.tk+1;return 25 end;return-2 end;return-1
    elseif action=="go_purple"then local pp=gUNT(true,false);if#pp==0 then return-1 end;local rw=0;INT=false;for _,ch in ipairs(pp)do if ch.part.Parent and not ch.col and not smT and not prec.nR and not ch.wasGreen then local ok=goTo(ch.part.Position,4,5);if ok==true and ch.part.Parent then ch.col=true;lP=ch.part;st.pr=st.pr+1;tL="🟣 1с";local t0=tick();while tick()-t0<PST do task.wait(0.05);if smT or prec.nR or not EN then break end end;rw=rw+30 end end end;return rw>0 and rw or-2
    elseif action=="go_tokenlink"then for p,t in pairs(aT)do if not t.col and p.Parent and t.p>=90 then tL="💎🔴 Link";INT=false;local ok=goTo(p.Position,5,5);if ok then t.col=true;igT=tick()+TLC;return 50 end;return-5 end end;return-2
    elseif action=="go_crosshair"then local all=gUNT(false,false);if#all==0 then all=gCH(false,false);if#all==0 then return-1 end end;local t=all[1];local rw=0;INT=false
...
-- === ГЛАВНЫЙ HEARTBEAT ===
local mLS=false
local function sML()if mLS then return end;mLS=true;task.wait(2);lQ();fAR();fF();lI("BSS AI v13.0!");print("✅ v13.0");tL="иниц";lMT=tick()end
...
R.Heartbeat:Connect(function()
    ...
    if hbF%18000==0 then pcall(function()local qc=0;for _ in pairs(QT)do qc=qc+1 end;pcall(writefile,"bss_ai_q_v13.json",H:JSONEncode({version=VQ,qtable=QT,meta={sc=qc,sa=os.time()}}))end)end
...
function lQ()local ok,raw=pcall(readfile,"bss_ai_q_v13.json");if ok and raw then local ok2,d=pcall(H.JSONDecode,H,raw);if ok2 and type(d)=="table"and d.version==VQ and type(d.qtable)=="table"then QT=d.qtable end end end
local function rQ()QT={};EP=0.1;st.tR=0;st.dc=0;pcall(writefile,"bss_ai_q_v13.json",H:JSONEncode({version=VQ,qtable={},meta={ra=os.time()}}))end

-- === GUI ===
local sg=Instance.new("ScreenGui",G)sg.Name="BSSAI_GUI"
local fr=Instance.new("Frame",sg)fr.Size=UDim2.new(0,260,0,105)fr.Position=UDim2.new(0,10,0,10)
...
local ti_=Instance.new("TextLabel",fr)ti_.Size=UDim2.new(1,0,0,20)ti_.Position=UDim2.new(0,0,0,2)ti_.BackgroundTransparency=1 ti_.Text="🧠 BSS AI v13.0"ti_.TextColor3=Color3.fromRGB(100,200,255)ti_.Font=Enum.Font.GothamBold ti_.TextSize=12 ti_.TextXAlignment=Enum.TextXAlignment.Center
...
task.spawn(function()while true do task.wait(0.3)lb.Text="🎯 "..tL;local cur=gHP();local hs=cur>0 and string.format("%.0f",cur)or"--";local bs=bAH>0 and string.format("%.0f",bAH)or"--";hl.Text="HPS: "..hs.." | Рек: "..bs;sl_.Text="⚡ "..string.format("%.0f",cS);pl_.Text="Φ:"..ph().." CH:"..#cQ.." Тк:"..st.tk.." Дг:"..st.diag end end)
