-- BSS AI v16.9 — Grid line detect, speed boost, XF0+SC24 mode, green CH -50
...
-- CROSSHAIR AVOIDANCE (v16.9 grid lines + hard strafe)
local cATCache,cATFrame={},0
local function gChSz(ch)local s=ch.part.Size;return math.max(s.X,s.Z,4)end
local function gRCT(mP,dP)if not prec.isX or prec.nR then return{}end;if cATFrame==hbF and cATCache[mP]then return cATCache[mP]end;local mf=Vector3.new(mP.X,0,mP.Z);local df=Vector3.new(dP.X,0,dP.Z);local tt=df-mf;if tt.Magnitude<1 then return{}end;local tD=tt.Unit;local th={}
 for i=1,#cQ do local ch=cQ[i];if not ch.col and ch.part.Parent and not ch.isP then local cf=Vector3.new(ch.part.Position.X,0,ch.part.Position.Z);local toCh=cf-mf;local d=toCh.Magnitude;local chSz=gChSz(ch);local effR=chSz*1.8+6
  if d<effR and d>0.2 then local dot=toCh.Unit:Dot(tD)
   if dot>-0.1 then local cross=math.abs(toCh.X*tD.Z-toCh.Z*tD.X)
    if cross<effR*0.85 then table.insert(th,{ch=ch,pos=cf,dist=d,cross=cross,sz=chSz})end end end end end
 -- TRUE GRID LINE DETECT: find rows/cols of 3+ CH, avoid entire line
 local lines={};local LINE_TOL=3.5;local LINE_MIN=3
 for i=1,#cQ do local a=cQ[i];if not a.col and a.part.Parent and not a.isP then
  -- Check X-aligned column
  local col={};for j=1,#cQ do local b=cQ[j];if not b.col and b.part.Parent and not b.isP and math.abs(a.part.Position.X-b.part.Position.X)<=LINE_TOL then table.insert(col,b)end end
  if#col>=LINE_MIN then local mnZ,mxZ=math.huge,-math.huge;for _,m in ipairs(col)do if m.part.Position.Z<mnZ then mnZ=m.part.Position.Z end;if m.part.Position.Z>mxZ then mxZ=m.part.Position.Z end end
   local midZ=(mnZ+mxZ)/2;local ext=(mxZ-mnZ)/2+8;local cx=a.part.Position.X
   local proj=Vector3.new(cx,0,midZ);local toP=proj-mf;local pd=toP.Magnitude
   if pd<50 and pd>0.2 then local dotL=toP.Unit:Dot(tD)
    if dotL>0.05 then table.insert(lines,{pos=proj,dist=pd,sz=ext,axis="Z"})end end end
  -- Check Z-aligned row
  local row={};for j=1,#cQ do local b=cQ[j];if not b.col and b.part.Parent and not b.isP and math.abs(a.part.Position.Z-b.part.Position.Z)<=LINE_TOL then table.insert(row,b)end end
  if#row>=LINE_MIN then local mnX,mxX=math.huge,-math.huge;for _,m in ipairs(row)do if m.part.Position.X<mnX then mnX=m.part.Position.X end;if m.part.Position.X>mxX then mxX=m.part.Position.X end end
   local midX=(mnX+mxX)/2;local ext=(mxX-mnX)/2+8;local cz=a.part.Position.Z
   local proj=Vector3.new(midX,0,cz);local toP=proj-mf;local pd=toP.Magnitude
   if pd<50 and pd>0.2 then local dotL=toP.Unit:Dot(tD)
    if dotL>0.05 then table.insert(lines,{pos=proj,dist=pd,sz=ext,axis="X"})end end end end end
 table.sort(lines,function(a,b)return a.dist<b.dist end)
 for _,l in ipairs(lines)do table.insert(th,{ch=cQ[1],pos=l.pos,dist=l.dist,cross=0,sz=math.min(l.sz,35)})end
 cATFrame=hbF;cATCache={};cATCache[mP]=th;return th end
local function cAT(mP,dP)local th=gRCT(mP,dP);if#th==0 then return nil end;table.sort(th,function(a,b)return a.dist<b.dist end);local t=th[1];local mf=Vector3.new(mP.X,0,mP.Z);local df=Vector3.new(dP.X,0,dP.Z);local tD=(df-mf).Unit;local toCh=t.pos-mf;local uCh=toCh.Unit
 local pushDist=math.max(22,t.sz*2+14)
 local p1=Vector3.new(-uCh.Z,0,uCh.X);local p2=Vector3.new(uCh.Z,0,-uCh.X)
 local bp=p2:Dot(tD)>=p1:Dot(tD)and p2 or p1
 st.chA=st.chA+1;return cP(Vector3.new(t.pos.X+bp.X*pushDist,mP.Y,t.pos.Z+bp.Z*pushDist))end
-- GREEN CH DETECT: CH turned green = (17,134,19)
...
local function sPt()fP={};if not ENABLED or not curF then return end;local pt=W:FindFirstChild("Particles");if not pt then return end;local r=h();if not r then return end;for _,o in ipairs(pt:GetChildren())do if o.Name=="PetalPart"and o:IsA("BasePart")and iF(o.Position)then local cn=gPC(o);if cn and PP[cn]then table.insert(fP,{part=o,cn=cn,pr=PP[cn],dist=d3d(r.Position,o.Position)})end end end;if redPT>0 then local rem=8.0-(tick()-redPT);if rem>0 and rem<4.0 then for _,fp in ipairs(fP)do if fp.cn=="Red"then fp.pr=0 end end elseif rem<=0 then redPT=0 end;table.sort(fP,function(a,b)if a.pr~=b.pr then return a.pr<b.pr end;return a.dist<b.dist end)end end
...
local function sSS()if not writefile then return end;if tick()-lastPS<120 then return end;lastPS=tick();rT10();pcall(function()writefile("bss_ai_scorch_v16.json",H:JSONEncode({scorchSessions=scorchSessions,bestScorch=bestSH,top10=top10}))end)end
...
-- ACTION BUILDER (v16.9 — grid lines, speed boost, XF0+SC24)
local function gAWB()
 ...
 -- 3: Super Scorch — FIXED priority chain
 if isSS then
  ...
  -- scorchAllCHMode: loot ALL crosshairs until scorch ends (v16.9)
  ...
  -- Purple window: first 3 purples OR 1s timeout → all-CH
  if not scorchAllCHMode then
   if scorchPurpleCount>=3 then scorchAllCHMode=true;scorchPurpleTime=tick()
    for i=1,#cQ do cQ[i].col=false end
    local all=gCH(false,false,true);if#all>0 then return{"go_crosshair_all"}end
    local path=gFCPath(r.Position);if path then return{"go_flame_path"}end
    return{"patrol_scorch_flames"}end
   if scorchPurpleTime>0 and n-scorchPurpleTime>=1 then scorchAllCHMode=true;scorchPurpleTime=tick()
    for i=1,#cQ do cQ[i].col=false end
    local all=gCH(false,false,true);if#all>0 then return{"go_crosshair_all"}end
    local path=gFCPath(r.Position);if path then return{"go_flame_path"}end
    return{"patrol_scorch_flames"}end
   local pp=gCH(true,false,false);if#pp>0 then return{"go_purple_scorch"}end end
  -- TP↔CH cycle during scorch
 ...
 -- 6b: DUPED MORPH priority (before other duped checks)
 ...
 -- 10: 9+ duped priority tokens — FIX: loot more aggressively
 if dupCnt>=9 then
 ...
 -- 11c: XF=0 + SC>=24 — loot ALL CH until XF>=10
 ...
 if p_=="REFRESH"then local all=gCH(false,false,true);if#all>0 then return{"go_crosshair_refresh_all"}end;return{"patrol_ring"}end
 if p_=="X10"then return{"patrol_ring"}end
 -- NABOR priority: TL > TP > HM/PM > MS > RB/BB/WB > random
 if p_=="NABOR"then
  if hTL()then return{"go_tokenlink"}end
  local tpBuild=gTPG_build();if tpBuild then return{"go_build_precision"}end
  -- TP priority
  for p,t in pairs(aT)do if not t.col and p.Parent and t.id==TPI and not t.dp then return{"go_token_best"}end end
  -- HM/PM priority
  for p,t in pairs(aT)do if not t.col and p.Parent and(t.id==2499514197 or t.id==2499540966)then tL="HM/PM";INT=false;if goTo(p.Position,5,5)and p.Parent then t.col=true;st.tk=st.tk+1 end;break end end
  -- MS priority
  for p,t in pairs(aT)do if not t.col and p.Parent and t.id==4528379338 then tL="MS";INT=false;if goTo(p.Position,5,5)and p.Parent then t.col=true;st.tk=st.tk+1 end;break end end
  -- Boost priority: RB/BB/WB
  for p,t in pairs(aT)do if not t.col and p.Parent and(t.id==1442859163 or t.id==1442863423 or t.id==3877732821)then tL="Boost";INT=false;if goTo(p.Position,5,5)and p.Parent then t.col=true;st.tk=st.tk+1 end;break end end
  local aCH=gCH(false,false,false);if#aCH>0 then table.insert(ba,"go_crosshair")end
...
local function dUQ(s,a,rw,ns)
 ...
 local prevXF=aB.XF.st;if xfP==0 and prevXF>=24 and scP>=27 then local cc=gFC();local rPos=rr and rr.Position;if cc~=Vector3.zero and rPos and d3(rPos,cc)<=XCR*1.5 then tR=tR+200 elseif cc~=Vector3.zero and rPos and d3(rPos,cc)<=XCR*4 then tR=tR+60 else tR=tR-50 end end
...
local function eA(action)
 ...
 if action=="go_flame_path"then local path=gFCPath(r.Position);if not path then return-1 end;tL="FPth";INT=false;local rw=0;for i=1,#path do local wp=path[i];if goTo(Vector3.new(wp.X,r.Position.Y,wp.Z),4,2.5)then rw=rw+5;local st_=tick();while tick()-st_<0.5 do wait(0.05);pcall(hitFlames);pcall(hitBloom)end else break end end;if rw>0 then local sc,_,_=gSC();if sc~=Vector3.zero then goTo(Vector3.new(sc.X,r.Position.Y,sc.Z),5,2)end end;return rw>0 and rw+5 or-2 end
 ...
 -- FIX: loot multiple duped tokens
 ...
 if action=="go_duped_tp_scorch"then for p,t in pairs(aT)do if not t.col and p.Parent and t.dp and t.id==TPI then tL="TP(Sc)";INT=false;if goTo(p.Position,4,3)and p.Parent then stToken(p);if aT[p]then aT[p].col=true end;-- FIX: after looting duped TP, set up CH cycle
  scorchTPTable={};scorchTPIndex=0;for p2,t2 in pairs(aT)do if not t2.col and p2.Parent and t2.dp and t2.id==TPI and p2~=p then table.insert(scorchTPTable,p2)end end;if#scorchTPTable>0 then scorchTPCycle=true;scorchTPIndex=0 end;return 25 end end end;return-2 end
 if action=="go_purple_scorch"then local pp=gCH(true,false,false);if#pp==0 then return-1 end;local ch=pp[1];if ch.part.Parent and not ch.col then tL="P ScC";INT=false;if goTo(ch.part.Position,4,3,true)and ch.part.Parent then ch.col=true;st.pr=st.pr+1;scorchPurpleCount=scorchPurpleCount+1;scorchPurpleTime=tick();return 20 end end;return-2 end
 ...
 -- FIX: coconut — stand until disappears (like crosshair)
 if action=="go_coconut"then local n=tick();for i,coco in ipairs(activeCocos)do if not coco.collected and coco.part.Parent and(3.0-(n-coco.spawnTime))<=1.0 then tL="Coco";INT=false;local hh=hm();local origSp=cS;if hh then hh.WalkSpeed=cS+10 end
  if goTo(coco.part.Position,3,2)and coco.part.Parent then if hh then hh.WalkSpeed=origSp end;local t0=tick();while coco.part.Parent do wait(0.05);pcall(hitBloom);pcall(hitFlames);if INT or not ENABLED then break end;local h__=hm();if h__ then h__:MoveTo(Vector3.new(coco.part.Position.X,coco.part.Position.Y,coco.part.Position.Z))end;if tick()-t0>60 then break end end;coco.collected=true;return 30 end
  if hh then hh.WalkSpeed=origSp end end end;return-2 end
 ...
 if action=="go_smile"then local sm=smT;if not sm or not sm.Parent then smT=nil;return-1 end;local td=aT[sm];if not td or td.col then smT=nil;return-1 end;isCS=true;tL="Sm";INT=false;goSm=true;for fl,data in pairs(scytheParts)do if fl and fl.Parent then if not(fl.Name or""):find("Dark")and sBC(fl)~="Really black"and d3(r.Position,fl.Position)<=SCYTHE_DIST*2 then pcall(hitFlames)end end end
 local hh=hm();local origSp=cS;-- check pink petal on way to smile
 for _,fp in ipairs(fP)do if fp.cn=="Pink"and fp.part.Parent then local dp=d3(r.Position,sm.Position);local fpDist=d3(r.Position,fp.part.Position);if fpDist<dp+10 then hh.WalkSpeed=cS+10;tL="Sm+10";if goTo(Vector3.new(fp.part.Position.X,0,fp.part.Position.Z),PCD,1.5)then st.pt=st.pt+1 end;break end end end
 -- stand in ring center first
 if aR and aR.Parent then goTo(aR.Position,5,2);local wt=tick();while tick()-wt<0.3 do wait(0.05);local h__=hm();if h__ then h__:MoveTo(Vector3.new(aR.Position.X,aR.Position.Y,aR.Position.Z))end end end
 local to=math.max(0.5,math.min(3,smTR-0.3));local ok=goTo(sm.Position,4,to);goSm=false;if hh then hh.WalkSpeed=origSp end
 if ok and sm.Parent then if not td.dp then td.col=true;smT=nil;st.sm=st.sm+1;isCS=false;dupCnt=0;for _,tt in pairs(aT)do if tt.dp and not tt.col then dupCnt=dupCnt+1 end end;return 45 end;local st_=tick();while tick()-st_<TSD do wait(0.1);if not sm.Parent then break end;local h__=hm();if h__ then h__:MoveTo(Vector3.new(sm.Position.X,sm.Position.Y,sm.Position.Z))end end;td.col=true;smT=nil;st.sm=st.sm+1;isCS=false;dupCnt=0;for _,tt in pairs(aT)do if tt.dp and not tt.col then dupCnt=dupCnt+1 end end;return 45 end;isCS=false;smT=nil;goSm=false;if hh then hh.WalkSpeed=origSp end;return-10 end
 -- FIX: purple — go directly, no circling
 if action=="go_purple"then local pp=gCH(true,false,false);if#pp==0 then return-1 end;local rw=0;INT=false;for i=1,#pp do local ch=pp[i];if ch.part.Parent and not ch.col and not smT then local isLast=(i==#pp);if isLast then tL="P stand";if goTo(ch.part.Position,3,6,true)and ch.part.Parent then ch.col=true;lP=ch.part;st.pr=st.pr+1;if prec.isX and not prec.nR then local t0=tick();while ch.part.Parent and tick()-t0<8 do wait(0.05);if smT or prec.nR then break end;local h__=hm();if h__ then h__:MoveTo(Vector3.new(ch.part.Position.X,ch.part.Position.Y,ch.part.Position.Z))end end end;rw=rw+20 end else if goTo(ch.part.Position,3,3,true)and ch.part.Parent then ch.col=true;lP=ch.part;st.pr=st.pr+1;tL="P run";rw=rw+15 end end end end;return rw>0 and rw or-2 end
 ...
 if action=="go_petal_cluster"then local cl=gPetCluster();if not cl then return-1 end;local c=cl[1];tL="PtCl"..c.size;INT=false;local hh=hm();local origSp=cS;if hh then hh.WalkSpeed=cS+10 end
  if goTo(c.center,5,3)then local rw=0;for _,m in ipairs(c.members)do if m.part.Parent then if goTo(Vector3.new(m.part.Position.X,0,m.part.Position.Z),PCD,1.2)then st.pt=st.pt+1;rw=rw+6+(14-m.pr);if m.cn=="Red"then redPT=tick()end end end end;if hh then hh.WalkSpeed=origSp end;return rw>0 and rw or 3 end
  if hh then hh.WalkSpeed=origSp end;return-2 end
...
-- INIT
local mLS=false
local function sML()if mLS then return end;mLS=true;wait(2);scriptStartH=getHoney()or 0;scriptStartT=tick();lQ();fAR();fF();lo("BSS AI v16.9 ready! "..fmtH(scriptStartH));print("v16.9 — grid lines, speed boost, XF0SC24, green CH -50");tL="init";lMT=tick()end
spawn(function()pcall(function()if not readfile then return end;local ok,raw=pcall(readfile,"bss_ai_pat_v16.json");if ok and raw then local ok2,data=pcall(H.JSONDecode,H,raw);if ok2 and type(data)=="table"then if data.scorchSessions then scorchSessions=data.scorchSessions end;if data.bestScorch then bestSH=data.bestScorch end end end end);lo("Loaded: "..#scorchSessions)end)
...
_G.BSSAI_HB=R.Heartbeat:Connect(function()
 ...
 if hbF%18000==0 then spawn(function()local qc=0;for _ in pairs(QT)do qc=qc+1 end;if writefile then pcall(function()writefile("bss_ai_q_v16.json",H:JSONEncode({version=Q_VERSION,qtable=QT,scorchSessions=scorchSessions,bestScorch=bestSH,top10=top10,eligibility=eligibility,visitCount=visitCount,totalSteps=totalSteps,meta={sc=qc,sa=os.time()}}))end)end end)end
 ...
 if prec.isX and not prec.nR and r then for i=1,#cQ do local ch=cQ[i];if ch.part and ch.part.Parent and not ch.col and not ch.isP and d3(r.Position,ch.part.Position)<4 then if isGreenCH(ch)then if n-lPT>1.5 then lPT=n;local s_=eS();if s_ and s_~="dead"then pcall(dUQ,s_,"patrol_ring",-80,s_)end;st.chA=st.chA+1;xGCH=xGCH+1;le("GREEN CH -80! total:"..xGCH)end end;break end end end
...
function lQ()if not readfile then return end;local ok,raw=pcall(readfile,"bss_ai_q_v16.json");if ok and raw then local ok2,d=pcall(H.JSONDecode,H,raw);if ok2 and type(d)=="table"and d.version==Q_VERSION and type(d.qtable)=="table"then QT=d.qtable;if d.scorchSessions then scorchSessions=d.scorchSessions end;if d.bestScorch then bestSH=d.bestScorch end;if d.top10 then top10=d.top10 end;if d.eligibility then eligibility=d.eligibility end;if d.visitCount then visitCount=d.visitCount end;if d.totalSteps then totalSteps=d.totalSteps end end end end
local function rQ()QT={};EP=0.1;st.tR=0;st.dc=0;scorchSessions={};bestSH=0;top10={};eligibility={};visitCount={};totalSteps=0;if writefile then pcall(function()writefile("bss_ai_q_v16.json",H:JSONEncode({version=Q_VERSION,qtable={},scorchSessions={},bestScorch=0,top10={},eligibility={},visitCount={},totalSteps=0,meta={ra=os.time()}}))end)end end
...
local tg=Instance.new("TextLabel",fr);tg.Size=UDim2.new(1,0,0,18);tg.Position=UDim2.new(0,0,0,2);tg.BackgroundTransparency=1;tg.Text="BSS AI v16.9";tg.TextColor3=Color3.fromRGB(100,200,255);tg.Font=Enum.Font.GothamBold;tg.TextSize=12;tg.TextXAlignment=Enum.TextXAlignment.Center
...
U.InputBegan:Connect(function(i,gp)if gp then return end;if i.KeyCode==Enum.KeyCode.T then ENABLED=not ENABLED elseif i.KeyCode==Enum.KeyCode.G then rQ()elseif i.KeyCode==Enum.KeyCode.P then local c=0;for _ in pairs(QT)do c=c+1 end;print("v16.9 eps:"..string.format("%.3f",EP).." Honey:"..fmtH(getHoney()))end end)
L.CharacterAdded:Connect(function()
 ...
 scorchActive=false;scorchRecording=false;scorchActions={};scorchStartH=0;scorchStartT=0;fixedXFC=nil;lastTLT=0;xfP=0;scP=0;lastBH=0;redPT=0;stFlMin=0;xGCH=0;lastSOTime=0;scorchDupedMorphDone=false;scorchPurpleCount=0;scorchPurpleTime=0;scorchAllCHMode=false;ndMorph=nil;scorchTPIndex=0;scorchTPTable={};scorchTPCycle=false
 ...
 spawn(function()if writefile then local qc=0;for _ in pairs(QT)do qc=qc+1 end;pcall(function()writefile("bss_ai_q_v16.json",H:JSONEncode({version=Q_VERSION,qtable=QT,scorchSessions=scorchSessions,bestScorch=bestSH,top10=top10,eligibility=eligibility,visitCount=visitCount,totalSteps=totalSteps,meta={sc=qc,sa=os.time()}}))end)end end)
end)
print("BSS AI v16.9 — grid lines, speed boost +10, XF0+SC24 mode, green CH -50 penalty")
