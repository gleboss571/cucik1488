-- BSS AI v16.5 FULL - Purple CH Priority Fix + All Features
...
local G=L:WaitForChild("PlayerGui");local ENABLED=true;local ELA=true;local SV=true
...
PURP=Color3.fromRGB(119,85,255);PTOL=20;SMI=5877939956;TSD=1.1
CAR=28;CAS=20;CAD=0.1;ARR=20;TLID=20;PCD=8;PFM=20;TPI=8173559749
...
local function gCH()local cs=L:FindFirstChild("CoreStats");if cs then local hv=cs:FindFirstChild("Honey");if hv then return hv.Value or 0 end end;return 0 end
...
fP={};aR=nil;aRR=ARR
aB={SS={st=0},XF={st=0},PM={a=false},PoM={a=false,pos=nil,m=0},PollM={combo=0,active=false,ringPos=nil}}
...
local function getCK()local xfB=0;if xfP>=22 then xfB=3 elseif xfP>=18 then xfB=2 elseif xfP>=10 then xfB=1 end;return string.format("%s|PM%d|PLL%d|XF%d",ph(),math.min(3,aB.PoM.m),math.min(1,pollMS>=3 and 1 or 0),xfB)end
...
local function iP(p)
 ...
 local ok3,bcn=pcall(function()return p.BrickColor.Name end);if ok3 and bcn then if bcn=="Lavender"or bcn=="Bright violet"or bcn=="Alder"then return true end end
 return false
end
local function aCH(o)if o.Name~="Crosshair"or not o:IsA("BasePart")then return end;for i=1,#cQ do if cQ[i].part==o then return end end;local isP=iP(o);table.insert(cQ,{part=o,sT=tick(),col=false,isP=isP});if not isP then for _,delay in ipairs({0.03,0.06})do task.spawn(function()task.wait(delay);if o.Parent then for i=1,#cQ do if cQ[i].part==o and not cQ[i].isP then local ck=iP(o);if ck then cQ[i].isP=true;break end end end end end)end end end
...
local function tryR(origTarget)local r=h();if not r then return false end;local onlyP=(prec.isX and not prec.nR);local best,bestD=nil,math.huge;for i=1,#cQ do local ch=cQ[i];if not ch.col and ch.part.Parent then if not(onlyP and not ch.isP)then local d=d3(r.Position,ch.part.Position);if d<=PCHR and d<bestD then bestD=d;best=ch end end end end;if not best then return false end;if origTarget and best.part==origTarget then return false end;local hm_=hm();if not hm_ then return false end;hm_:MoveTo(best.part.Position);local t0=tick();while tick()-t0<0.6 do task.wait(0.03);local r2=h();if not r2 then break end;if not best.part.Parent then best.col=true;break end;if d3(r2.Position,best.part.Position)<=4 then best.col=true;st.chP=st.chP+1;if best.isP then st.pr=st.pr+1;lP=best.part else st.ch=st.ch+1 end;return true end end;if best.part.Parent then best.col=true;st.chP=st.chP+1;if best.isP then st.pr=st.pr+1 else st.ch=st.ch+1 end end;return true end
...
local function hitFlames()
 local r=h();if not r then return end;local n=tick();if n-lastSHit<SCYTHE_CD then return end
 for fl,data in pairs(scytheParts)do if fl and fl.Parent then local nm=fl.Name or"";local bn=sBC(fl);local isD=(nm:find("Dark")or bn=="Really black");local cd=flameCD[fl];if not isD and(not cd or n>=cd)then local dist=d3(r.Position,fl.Position);if dist<=SCYTHE_DIST*2 and data and(n-data.sT)>=6.0 then lastSHit=n;flameCD[fl]=n+5.0;flHit=flHit+1;stFlMin=stFlMin+1;local bg=r:FindFirstChild("BG_S");if not bg then bg=Instance.new("BodyGyro");bg.Name="BG_S";bg.MaxTorque=Vector3.new(0,40000,0);bg.P=10000;bg.D=500;bg.Parent=r end;local dir=fl.Position-r.Position;dir=Vector3.new(dir.X,0,dir.Z);if dir.Magnitude>0.1 then bg.CFrame=CFrame.lookAt(r.Position,r.Position+dir)end;local ev=RS:FindFirstChild("Events");local tce=ev and ev:FindFirstChild("ToolCollect");if tce then pcall(function()tce:FireServer()end)else pcall(function()local cam=workspace.CurrentCamera;local vp=cam.ViewportSize;V:SendMouseButtonEvent(vp.X/2,vp.Y/2,0,true,game,1);V:SendMouseButtonEvent(vp.X/2,vp.Y/2,0,false,game,1)end)end;task.spawn(function()task.wait(0.15);if bg then bg:Destroy()end end);break end end else scytheParts[fl]=nil;flameCD[fl]=nil end end
end
...
local function goTo(tP,rad,to,sk)
 ...
 hm_:MoveTo(cM);local t0=tick();local lM=tick();local lA=tick();local lPC=tick();local su=false;local at=tick();local al=r.Position
 while tick()-t0<to do
  ...
  if hbF%3==0 and not su then
   ...
   if bst and not INT then su=true;local dp=bst.Position;local hd=hm();if hd then hd:MoveTo(Vector3.new(dp.X,r.Position.Y,dp.Z))end;local td=tick();while tick()-td<0.8 do task.wait(0.04);if INT then break end;local rd=h();if not rd then break end;if d3(rd.Position,dp)<5 then if aT[bst]then aT[bst].col=true;st.tk=st.tk+1 end;chStick=chStick+1;break end end;hm_=hm();if hm_ then hm_:MoveTo(cM)end end
  end
...
local function rT(o)if o.Name~="C"or not o:IsA("BasePart")or aT[o]or tick()<igT then return end;local fr=o:FindFirstChild("FrontDecal");if not fr or not fr:IsA("Decal")then return end;local id=ti(fr.Texture);if not id or AV[id]then return end;local df=TKS[id];if not df then return end;local r=h();local dp=false;if r then dp=(o.Position.Y-r.Position.Y)>3 end;local lf=df.base*AM;if dp then lf=lf*(2+0.05*(DGL-1));dupCnt=dupCnt+1 end;aT[o]={id=id,n=df.n,p=df.p,mo=df.mo or false,s=tick(),l=lf,dp=dp,col=false};if dp and id==1629547638 then return end;cT(o,id,lf,dp,df)end
...
local function pAB()
 ...
 -- FIXED: aBollM -> aB.PollM (line ~160 syntax fix)
...
local function gAWB()
 ...
 local cs=L:FindFirstChild("CoreStats");if cs then local cap=cs:FindFirstChild("Capacity");local pol=cs:FindFirstChild("Pollen");if cap and pol and cap.Value>0 and(pol.Value/cap.Value)>=0.9 then local all=gCH(false,false,false);if#all>0 then return{"go_backpack_dump"}end end end
 local r=h()
 ...
 if p_=="X10"and not isSO then local pp=gCH(true,false,false);if#pp>0 then return{"go_purple"}end;local aCH=gCH(false,false,false);if#aCH>=2 then task.wait(0.45);pp=gCH(true,false,false);if#pp>0 then return{"go_purple"}end end end
 local nearCH=gCH_n();if nearCH and not smT and not xfE and not isSS and not isSO then return{"go_crosshair"}end
 if isSS then local sc,_,_=gSC();if sc~=Vector3.zero and r then local bt,bd=nil,math.huge;for p,t in pairs(aT)do if not t.col and p.Parent and t.p>=8 then local d=d3(p.Position,sc);if d<40 and d<bd then bd=d;bt=p end end end;if bt then return{"go_scorch_token"}end end;local all=gCH(false,false,true);if#all>0 then return{"go_crosshair_all"}end;return{"patrol_scorch_flames"}end
 if xfE then local cc=gFC();if cc~=Vector3.zero then local bC=gCH_nc();if bC then return{"go_xflame_ch"}end;return{"go_xflame_center"}end end
 if isSc then for p,t in pairs(aT)do if not t.col and p.Parent and t.id==2000457501 and t.dp and(n-t.s)>2.0 then return{"go_duped_inspire_scorch"}end end
 ...
 for i,coco in ipairs(activeCocos)do if not coco.collected and coco.part.Parent then if(3.0-(n-coco.spawnTime))<=1.0 then return{"go_coconut"}end end end
...
-- STAND ON CH
local function soCH(ch,dur)local r=h();local hm_=hm();if not r or not hm_ then return end;hm_:MoveTo(Vector3.new(ch.part.Position.X,r.Position.Y,ch.part.Position.Z));local t0=tick();while tick()-t0<dur do task.wait(0.05);if not ch.part.Parent then break end end;if ch.part.Parent and not ch.col then ch.col=true;st.chP=st.chP+1;if ch.isP then st.pr=st.pr+1 else st.ch=st.ch+1 end end end

-- EXECUTE
local function eA(action)
 local r=h();if not r then return-1 end;tL=action;rSA(action)
 if action=="go_smile"then for fl,data in pairs(scytheParts)do if fl and fl.Parent then local nm=fl.Name or"";local bn=sBC(fl);if not(nm:find("Dark")or bn=="Really black")and d3(r.Position,fl.Position)<=SCYTHE_DIST*2 then pcall(hitFlames)end end end end
 if action=="patrol_scorch_flames"then local sc,hf,dn=gSC();tL=hf and(dn>=5 and"SS Tight"or dn>=2 and"SS Close"or"SS Orbital")or"SS Patrol";INT=false;if sc~=Vector3.zero and hf then local oa=(tick()*0.8)%(2*math.pi);local or2=dn>=5 and SCYTHE_DIST*0.5 or dn>=2 and SCYTHE_DIST*0.7 or SCYTHE_DIST*0.9;goTo(Vector3.new(sc.X+math.cos(oa)*or2,r.Position.Y,sc.Z+math.sin(oa)*or2),4,PT)elseif sc~=Vector3.zero then local ang=math.random()*2*math.pi;goTo(Vector3.new(sc.X+math.cos(ang)*math.random()*aRR*0.5,r.Position.Y,sc.Z+math.sin(ang)*math.random()*aRR*0.5),5,PT)else goTo(r.Position,5,2)end;task.wait(0.1+math.random()*0.2);return 0 end
 ...
 if action=="go_backpack_dump"then local all=gCH(false,false,false);if#all==0 then return-1 end;local best,bd=nil,math.huge;for _,ch in ipairs(all)do local d=d3(r.Position,ch.part.Position);if d<bd then bd=d;best=ch end end;tL="90% dump";INT=false;if goTo(best.part.Position,4,3)and best.part.Parent then local t0=tick();while tick()-t0<1.3 do task.wait(0.05);if not best.part.Parent then break end;local h__=hm();if h__ then h__:MoveTo(Vector3.new(best.part.Position.X,best.part.Position.Y,best.part.Position.Z))end end;best.col=true;st.chP=st.chP+1;if best.isP then st.pr=st.pr+1 else st.ch=st.ch+1 end;return 50 end;return-2 end
 if action=="go_shower"then for i=1,#activeShowers do local sh=activeShowers[i];if not sh.collected and sh.part.Parent and(tick()-sh.spawnTime)<0.8 then tL="Shower";INT=false;r.CFrame=CFrame.new(sh.part.Position.X,sh.part.Position.Y+3,sh.part.Position.Z);local t0=tick();while tick()-t0<2.0 do task.wait(0.05);if not sh.part.Parent then break end end;sh.collected=true;local nf=true;while nf and not INT do nf=false;for j=1,#activeShowers do local ns=activeShowers[j];if not ns.collected and ns.part.Parent and ns~=sh and(tick()-ns.spawnTime)<2.5 then tL="Shower TP";r.CFrame=CFrame.new(ns.part.Position.X,ns.part.Position.Y+3,ns.part.Position.Z);local t1=tick();while tick()-t1<2.0 do task.wait(0.05);if not ns.part.Parent then break end end;ns.collected=true;nf=true;break end end end;return 10+math.min(40,#activeShowers*10)end end;return-1 end
 if action=="go_duped_morph"then local n=tick();for p,t in pairs(aT)do if not t.col and p.Parent and t.dp and t.mo and(t.l-(n-t.s))<4.0 then tL="Morph(D)";INT=false;if goTo(p.Position,4,3)and p.Parent then local t0=tick();while tick()-t0<1.1 do task.wait(0.05);if not p.Parent then break end;local h__=hm();if h__ then h__:MoveTo(Vector3.new(p.Position.X,p.Position.Y,p.Position.Z))end end;t.col=true;return 25 end end end;return-2 end
 if action=="go_duped_babylove"then local n=tick();for p,t in pairs(aT)do if not t.col and p.Parent and t.dp and t.id==1472256444 and(t.l-(n-t.s))<5.0 then tL="BL(D)";INT=false;if goTo(p.Position,4,3)and p.Parent then local t0=tick();while tick()-t0<1.1 do task.wait(0.05);if not p.Parent then break end;local h__=hm();if h__ then h__:MoveTo(Vector3.new(p.Position.X,p.Position.Y,p.Position.Z))end end;t.col=true;return 25 end end end;return-2 end
 if action=="go_duped_inspire_scorch"then local n=tick();for p,t in pairs(aT)do if not t.col and p.Parent and t.dp and t.id==2000457501 and(n-t.s)>2.0 then tL="IN(Sc)";INT=false;if goTo(p.Position,4,3)and p.Parent then local t0=tick();while tick()-t0<1.1 do task.wait(0.05);if not p.Parent then break end;local h__=hm();if h__ then h__:MoveTo(Vector3.new(p.Position.X,p.Position.Y,p.Position.Z))end end;t.col=true;return 35 end end end;return-2 end
 if action=="go_duped_inspire_normal"then local n=tick();for p,t in pairs(aT)do if not t.col and p.Parent and t.dp and t.id==2000457501 and(t.l-(n-t.s))<2.0 then tL="IN(D)";INT=false;if goTo(p.Position,4,3)and p.Parent then local t0=tick();while tick()-t0<1.1 do task.wait(0.05);if not p.Parent then break end;local h__=hm();if h__ then h__:MoveTo(Vector3.new(p.Position.X,p.Position.Y,p.Position.Z))end end;t.col=true;return 25 end end end;return-2 end
 ...
 if action=="go_smile"then if not smT or not smT.Parent then smT=nil;return-1 end;local td=aT[smT];if not td or td.col then smT=nil;return-1 end;isCS=true;tL="Sm";INT=false;goSm=true;for fl,data in pairs(scytheParts)do if fl and fl.Parent then if not(fl.Name or""):find("Dark")and sBC(fl)~="Really black"and d3(r.Position,fl.Position)<=SCYTHE_DIST*2 then pcall(hitFlames)end end end;local to=math.max(0.5,math.min(3,smTR-0.3));local ok=goTo(smT.Position,4,to);goSm=false;if ok and smT.Parent then if not td.dp then td.col=true;smT=nil;st.sm=st.sm+1;isCS=false;dupCnt=0;for _,tt in pairs(aT)do if tt.dp and not tt.col then dupCnt=dupCnt+1 end end;return 45 end;local st_=tick();while tick()-st_<TSD do task.wait(0.1);if not smT.Parent then break end;local h__=hm();if h__ then h__:MoveTo(Vector3.new(smT.Position.X,smT.Position.Y,smT.Position.Z))end end;td.col=true;smT=nil;st.sm=st.sm+1;isCS=false;dupCnt=0;for _,tt in pairs(aT)do if tt.dp and not tt.col then dupCnt=dupCnt+1 end end;return 45 end;isCS=false;smT=nil;goSm=false;return-10 end
 if action=="go_purple"then local pp=gCH(true,false,false);if#pp==0 then return-1 end;local rw=0;INT=false;for i=1,#pp do local ch=pp[i];if ch.part.Parent and not ch.col and not smT then if goTo(ch.part.Position,5,5)and ch.part.Parent then ch.col=true;lP=ch.part;st.pr=st.pr+1;tL="P 1s";if prec.isX and not prec.nR then local t0=tick();while tick()-t0<1.0 do task.wait(0.05);if smT or prec.nR then break end;local h__=hm();if h__ then h__:MoveTo(Vector3.new(ch.part.Position.X,ch.part.Position.Y,ch.part.Position.Z))end end end;rw=rw+20 end end end;return rw>0 and rw or-2 end
 if action=="go_tokenlink"then local tl={};local n_=tick();for p,t in pairs(aT)do if not t.col and p.Parent and t.p>=90 then table.insert(tl,{p=p,t=t,rem=t.l-(n_-t.s)})end end;if#tl==0 then return-2 end;table.sort(tl,function(a,b)return a.rem<b.rem end);local bT=nil;for _,e in ipairs(tl)do local skip=false;if aB.XF.st>0 then local cc=gFC();if cc~=Vector3.zero then if d3(e.p.Position,cc)>XCR*2 then if aB.XF.st+cBT(e.p.Position,40)>=25 then skip=true end end end end;if not skip then bT=e;break end end;if not bT then return-2 end;tL="Link";INT=false;if goTo(bT.p.Position,5,5)and bT.p.Parent then bT.t.col=true;igT=tick()+TLC;lastTLT=tick();for _,e2 in ipairs(tl)do if e2.p~=bT.p and e2.p.Parent and not e2.t.col then local dist2=d3(bT.p.Position,e2.p.Position);if dist2<40 and e2.rem>3.0 then task.wait(math.min(2.5,e2.rem-1.0));if e2.p.Parent and not e2.t.col then goTo(e2.p.Position,5,4);if e2.p.Parent then e2.t.col=true end end elseif dist2<40 and e2.rem<=3.0 then goTo(e2.p.Position,5,4);if e2.p.Parent then e2.t.col=true end end;break end end;return 50 end;return-5 end
 ...
 if action=="go_petal"then if#fP==0 then return-1 end;local ca=false;local tr=0;local i=1;while i<=#fP do local pt=fP[i];if not pt.part.Parent then table.remove(fP,i)elseif stP[pt.part]and tick()<stP[pt.part]then table.remove(fP,i)else tL=pt.cn;INT=false;if goTo(Vector3.new(pt.part.Position.X,0,pt.part.Position.Z),PCD,2.5)then st.pt=st.pt+1;tr=tr+8+(14-pt.pr);ca=true;if pt.cn=="Red"then redPT=tick()end;task.wait(0.05);i=i+1 else stP[pt.part]=tick()+5;table.remove(fP,i)end end end;return ca and tr or-1 end
 ...
 if action=="go_token_best"then local bs=cS+5;local be=nil;local br=0;local n=tick();local cand={};for p,t in pairs(aT)do if not t.col and p.Parent then local rem=t.l-(n-t.s);if rem>0.5 then local sc2=t.p*(rem/t.l);if t.dp then sc2=sc2*1.3 end;table.insert(cand,{p=p,t=t,score=sc2,rem=rem})end end end;if#cand==0 then return-1 end;table.sort(cand,function(a,b)return a.score>b.score end);be=cand[1].p;br=cand[1].rem;local s2=nil;if#cand>=2 and r then local d1=d3(r.Position,cand[1].p.Position);local d12=d3(cand[1].p.Position,cand[2].p.Position);local mr=math.min(cand[1].rem,cand[2].rem);if d1/cS+d12/cS<mr then s2=cand[2]else local t1b=d1/bs;local t2b=d12/bs;if t1b+t2b<mr then s2=cand[2];local h_=hm();if h_ then h_.WalkSpeed=bs end;task.spawn(function()task.wait(math.min(4,t1b+t2b+0.5));local h__=hm();if h__ then h__.WalkSpeed=cS end end)end end end;if s2 then tL=aT[be].n.."+"..aT[s2.p].n;INT=false;if goTo(be.Position,4,4)and be.Parent then aT[be].col=true;st.tk=st.tk+1;if goTo(s2.p.Position,4,4)and s2.p.Parent then aT[s2.p].col=true;st.tk=st.tk+1;return 10+(aT[be].p+aT[s2.p].p)*0.2 end;return 5+aT[be].p*0.3 end;return-3 end;if be then local d1=d3(r.Position,be.Position);if d1/cS>br and d1/bs<br then local h_=hm();if h_ then h_.WalkSpeed=bs end;task.spawn(function()task.wait(math.min(4,d1/bs+0.5));local h__=hm();if h__ then h__.WalkSpeed=cS end end)end;tL=aT[be].n;INT=false;if goTo(be.Position,5,5)and be.Parent then aT[be].col=true;return 5+aT[be].p*0.3 end;return-3 end;return-1 end
 ...
 if action=="go_xflame_center"then local c=gFC();if c==Vector3.zero then return-1 end;if xfP>=20 then local ch=gCH_nc();if ch then tL="XF CH";INT=false;soCH(ch,1.0);return 5 end end;tL="XF c";INT=false;goTo(c,3,3);return 0 end
...
-- INIT
local mLS=false
local function sML()if mLS then return end;mLS=true;task.wait(2);scriptStartH=gCH();scriptStartT=tick();lQ();fAR();fF();lo("BSS AI v16.5 ready! "..fmtH(scriptStartH));print("v16.5");tL="init";lMT=tick()end
task.spawn(function()pcall(function()if not readfile then return end;local ok,raw=pcall(readfile,"bss_ai_pat_v15.json");if ok and raw then local ok2,data=pcall(H.JSONDecode,H,raw);if ok2 and type(data)=="table"then if data.scorchSessions then scorchSessions=data.scorchSessions end;if data.bestScorch then bestSH=data.bestScorch end end end end);lo("Loaded: "..#scorchSessions)end)
...
_G.BSSAI_HB=R.Heartbeat:Connect(function()
 ...
 if hbF%3==0 then sSm()end;if hbF%180==0 then fF()end;if hbF%60==0 then clnCH()end;if hbF%30==0 then sSS()end
 if hbF%36000==0 then for _,tbl in pairs(qTables)do for k,v in pairs(tbl)do tbl[k]=v*0.99 end end end
 if hbF%18000==0 then task.spawn(function()local qc=0;for _ in pairs(QT)do qc=qc+1 end;if writefile then pcall(function()writefile("bss_ai_q_v15.json",H:JSONEncode({version=Q_VERSION,qtable=QT,scorchSessions=scorchSessions,bestScorch=bestSH,top10=top10,eligibility=eligibility,visitCount=visitCount,totalSteps=totalSteps,meta={sc=qc,sa=os.time()}}))end)end end)end
 xfE=(aB.XF.st>=19);if xfE then INT=true;if not fixedXFC then local tf=W.Flowers:FindFirstChild("FP18-10-13");if tf then fixedXFC=tf.Position end end else fixedXFC=nil end;uVC()
...
local xl=Instance.new("TextLabel",fr);xl.Size=UDim2.new(1,0,0,14);xl.Position=UDim2.new(0,0,0,90);xl.BackgroundTransparency=1;xl.Text="XF:--/25 SS:--/30 PM:-- PLM:-- dup:--";xl.TextColor3=Color3.fromRGB(255,180,100);xl.Font=Enum.Font.Gotham;xl.TextSize=9;xl.TextXAlignment=Enum.TextXAlignment.Center
...
task.spawn(function()while true do task.wait(0.3);pcall(function()
 ...
 local curH=gCH();local em=(tick()-scriptStartT)/60;local hp40m=em>0 and(curH-scriptStartH)/em*40 or 0;hl.Text="HP40M: "..fmtH(hp40m).." | "..fmtH(curH)
 local pt=prec.isX and"[X10]"or("["..prec.st.."/10]");local ptt=prec.tL>0 and string.format("%.0fs",prec.tL)or"--";local pc=prec.isX and prec.tL<=PRAT and"!"or"";sl.Text="S: "..string.format("%.0f",cS).." | "..pc.."PREC: "..ptt.." "..pt
 pl_.Text=ph().." CH:"..#cQ.." Tk:"..st.tk.." Pr:"..st.pr.." Sm:"..st.sm
 local cr=em>0 and math.floor(st.chP/em)or 0;local tr=em>0 and math.floor(st.tk/em)or 0;local flr=em>0 and math.floor(stFlMin/em)or 0;bf.Text="CH:"..cr.."/m Tk:"..tr.."/m Fl:"..flr.."/m"
 xl.Text="XF:"..xfP.."/25 SS:"..scP.."/30 PM:"..aB.PoM.m.." PLM:"..pollMS.." dup:"..dupCnt
 if scorchActive and scorchStartT>0 then sh.Text=""..fmtH(curH-scorchStartH).." "..string.format("%.1f",(tick()-scorchStartT)/60).."min | "..fmtH(bestSH)else sh.Text=""..#scorchSessions.." scorches | T10:"..#top10.." | "..fmtH(bestSH)end
end)end end)
U.InputBegan:Connect(function(i,gp)if gp then return end;if i.KeyCode==Enum.KeyCode.T then ENABLED=not ENABLED elseif i.KeyCode==Enum.KeyCode.G then rQ()elseif i.KeyCode==Enum.KeyCode.P then local c=0;for _ in pairs(QT)do c=c+1 end;print("v16.5 eps:"..string.format("%.3f",EP).." Honey:"..fmtH(gCH()))end end)
L.CharacterAdded:Connect(function()
 task.wait(2);aT={};cQ={};lP=nil;curF=nil;tL="start";smT=nil;isCS=false;INT=false;cyc={chC=0};fP={};igT=0;rCC=0;dupCnt=0;pollMS=0;eligibility={};visitCount={};totalSteps=0
 scorchActive=false;scorchRecording=false;scorchActions={};scorchStartH=0;scorchStartT=0;fixedXFC=nil;lastTLT=0;xfP=0;scP=0;lastBH=0;redPT=0;stFlMin=0
 for _,v in pairs(activeTG)do if v.gui then pcall(function()v.gui:Destroy()end)end end;activeShowers={};activeBlooms={};activeCocos={};activeTG={}
 ...
 task.spawn(function()if writefile then local qc=0;for _ in pairs(QT)do qc=qc+1 end;pcall(function()writefile("bss_ai_q_v15.json",H:JSONEncode({version=Q_VERSION,qtable=QT,scorchSessions=scorchSessions,bestScorch=bestSH,top10=top10,eligibility=eligibility,visitCount=visitCount,totalSteps=totalSteps,meta={sc=qc,sa=os.time()}}))end)end end)
end)
print("BSS AI v16.5 - Purple CH Priority Fix - ALL FEATURES ACTIVE")
