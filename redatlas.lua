...
local function getSpiralTarget()
  ...
  local step=SPIRAL_STEP/math.max(sprR,1);sprA=sprA+step;sprR=1+sprA*SPIRAL_STEP/(2*math.pi)
  if sprR>SCRD*3 then sprA=0;sprR=1 end
  local tx=sprC.X+math.cos(sprA)*sprR;local tz=sprC.Z+math.sin(sprA)*sprR
...
local function uSC() if ENABLED then local r=h();if r then if not scC then scC=Instance.new("Part");scC.Name="ScytheRadius";scC.Shape=Enum.PartType.Cylinder;scC.Anchored=true;scC.CanCollide=false;scC.Transparency=0.6;pcall(function()scC.BrickColor=BrickColor.palette(106)end);scC.Size=Vector3.new(FHD*2,0.2,FHD*2);scC.Parent=Workspace end;scC.Position=Vector3.new(r.Position.X,r.Position.Y+0.2,r.Position.Z)end else if scC then scC:Destroy();scC=nil end end end
...
_G._BSSAI_HB=RunService.Heartbeat:Connect(function()
  ...
  if hbF%18000==0 then task.spawn(function()local qc=0;for _ in pairs(QT)do qc=qc+1 end;pcall(writefile,"bss_ai_q_v14.json",Http:JSONEncode({version=Q_VERSION,qtable=QT,meta={sc=qc,ep=math.floor(EP*1000)/1000,uid=tostring(LP.UserId),sa=os.time(),tR=math.floor(st.tR),dc=st.dc}}))end)end
